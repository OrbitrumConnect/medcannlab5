-- =====================================================
-- 🔧 CRIAÇÃO DE VIEWS E FUNÇÕES FALTANTES
-- =====================================================
...
-- Forçar remoção completa de views antigas usando bloco DO
DO $$
BEGIN
  -- Remover v_doctor_dashboard_kpis se existir (pode ter sido criada com erro)
  IF EXISTS (
    SELECT 1 FROM information_schema.views 
    WHERE table_schema = 'public' 
    AND table_name = 'v_doctor_dashboard_kpis'
  ) THEN
    EXECUTE 'DROP VIEW IF EXISTS v_doctor_dashboard_kpis CASCADE';
  ...
  IF EXISTS (
    SELECT 1 FROM information_schema.views 
    WHERE table_schema = 'public' 
    AND table_name = 'v_next_appointments'
  ) THEN
    EXECUTE 'DROP VIEW IF EXISTS v_next_appointments CASCADE';
    RAISE NOTICE 'View v_next_appointments removida';
  END IF;
END $$;

-- Garantir remoção adicional (caso o bloco acima não tenha funcionado)
...
-- =====================================================
-- PARTE 2: VIEW v_doctor_dashboard_kpis
-- =====================================================
...
-- =====================================================
-- PARTE 4: POLÍTICAS RLS PARA AS VIEWS
-- =====================================================

-- As views herdam as políticas RLS das tabelas base (appointments, notifications, chat_messages)
-- Mas podemos adicionar políticas específicas se necessário

-- Permitir que usuários autenticados vejam suas próprias estatísticas
-- (As views são apenas SELECT, então não precisam de políticas RLS específicas)
...
-- Testar a função RPC (substitua o UUID por um UUID válido do seu banco)
-- SELECT get_unread_notifications_count('00000000-0000-0000-0000-000000000000'::UUID);

-- Testar a view v_doctor_dashboard_kpis
SELECT * FROM v_doctor_dashboard_kpis LIMIT 1;

-- Testar a view v_next_appointments
...
-- ✅ View v_doctor_dashboard_kpis criada
