...
- **Estado (narrativa + doc):** ~**97%** (diário 30/03) é **meta narrativa** — **não** usar esse número para antecipar go-live; preferir mentalidade *staging maduro + gates pendentes* (**§9**, **9.D**, **9.C**). Gates reais = **§9** (Fases A–I + **9.D** smoke + tabela **9.B**).  
...
**Versão:** 2.6 — §9 (+ **9.D** smoke, decisões irreversíveis, **9.C** com severidade) + **§10.2** fonte de verdade por domínio  
**Compilação:** 4 de abril de 2026  
...
**Nota:** `DIARIO_31_03_2026.md` segue opcional; o **selo de 01/04** está em `DIARIO_01_04_2026.md`; continuidade **02/04** em `DIARIO_02_04_2026.md` (**§1–6** AEC/agenda; **§7** auditoria Supabase + scripts + pitch); **plano 03/04** em `DIARIO_03_04_2026.md`; **documentação de cores / design tokens (sem refactor)** em `DIARIO_04_04_2026.md`. **Nunca** colar credenciais ou chaves neste documento.
...
| **Nôa cita horários que não batem com o card** | Prompt do Core tinha dias/horas fixos; app usa **`get_available_slots_v3`**. Ver **`DIARIO_02_04_2026.md`** e bloco PERFIS em `tradevision-core/index.ts` (02/04/2026). |
| **Auditar conversa AEC / “onde está o chat?”** | Fluxo atual: **`patient_medical_records`** (`chat_interaction`); `ai_chat_interactions` pode estar vazio. Scripts: `database/scripts/AUDIT_CHAT_RAG_…`, `AUDIT_SUPABASE_NOA_ECOSYSTEM_READ_ONLY.sql`. Ver **`DIARIO_02_04_2026.md` §7**. |
| **SQL Editor Postgres** | Não usar `:nome` como placeholder; usar literais ou CTE `params`. Confirmar nomes de colunas no schema (ex. `noa_interaction_logs."timestamp"`, não `created_at`). |
...
9. **§9** — **Runbook de go-live** (Fases A–I; **9.D** smoke antes do beta; **9.A** snapshot; **9.B** deps; **9.C** riscos com severidade; bloco **decisões já tomadas**).  
10. **§10** — Realidade no repo + **§10.2** fonte de verdade por domínio (chat, relatório, agenda, vídeo).  
...
| 15 | `DIARIO_02_04_2026.md` | **02/04/2026** — AEC âncora/profissional de destino; prompt agendamento vs RPC; rota chat; **§7** auditoria Supabase; **§8** relatório Lovable (inventário) |
| 16 | `DIARIO_03_04_2026.md` | **03/04/2026** — plano: validar chat/IMRE sem quebrar; uma ligação por vez |
| 17 | `DIARIO_04_04_2026.md` | **04/04/2026** — auditoria de **cores** (Tailwind, `index.css`, marca `#00C16A`); decisão: **só documentar**, sem unificação agora; intenção tech + leitura |
| 18 | **`DIARIO_31_03_2026.md`** | *Opcional — último dia de março* |
| 18 | `docs/LIVRO_MAGNO_RESUMO_FINAL_09-02-2026.md` / `docs/LIVRO_MAGNO_COMPLETO_DETALHADO_09-02-2026.md` | 09/02 |
| 19 | `INVESTMENT_KIT/pitch_estilo_sequoia.md` | Pitch investidor (Sequoia-style) |
| 20 | `INVESTMENT_KIT/pitch_deck_medcannlab.md` | Deck narrativo |
| 21 | `acordo_quotistas_juridico.md` | Acordo quotistas v2 (IP, vesting, veto CTO) |
| 22 | `docs/O_LIVRO_MAGNO_DA_JORNADA_MEDCANNLAB.txt` | Narrativa longa (Vidal, Passos, jornada) |
| 23 | `docs/guides/TREINAMENTO_OPENAI_ASSISTANT.md` | Base metodológica / Valença — entrevista clínica |
...
`DIARIO_02_04_2026.md` — AEC (âncora HDA, profissional de destino, Core sem horários fixos duplicados); alinhamento prompt × `get_available_slots_v3`. **§7:** strip RAG na gravação; fallback modal relatórios; scripts `database/scripts/` (auditoria + backfill opcional); `noa_memories` vs `documents`; data room no INVESTMENT_KIT. **§8:** snapshot auditoria **Lovable ↔ Supabase** (inventário; limites do scan).  
`DIARIO_03_04_2026.md` — **plano operacional** (pré-execução): validar `chat_messages` vs legacy, rastrear persistência IMRE/AEC, uma alavanca (view/KPI) sem big bang; lista do que **não** fazer no mesmo dia.  
`DIARIO_04_04_2026.md` — **selo documental** (pós-conversa): inventário das paletas no front, dívidas (`accent` duplicado no Tailwind; `.card`/`.gradient-bg` duplicados em CSS), coexistência verde marca vs `primary-*`; **sem** alteração de código neste passo; direção **tech + legibilidade** para profissionais **registada**.
...
| 02/04 | AEC âncora + profissional escolhido na jornada; Core “secretária” sem grade fixa no prompt (verdade = RPC); rota chat paciente preserva `state`; **auditoria Supabase** — prontuário chat, scripts só leitura/backfill RAG, ecossistema `noa_*`, curadoria `documents`, alinhamento pitch; **§8** inventário Lovable | `DIARIO_02_04_2026.md` §1–8; `database/scripts/`; `LIVRO_MAGNO_DIARIO_UNIFICADO.md` (dois blocos 02/04) |
| 03/04 | *(plano)* Validar chat P2P vs `chat_messages`; rastrear IMRE/AEC sem duplicar persistência; no máximo **uma** ligação nova (view/KPI) com rollback simples | `DIARIO_03_04_2026.md` |
| 04/04 | **Documentação** — cores e tokens do app mapeados; decisão consciente de **não** unificar paleta ainda; selo de intenção visual (tech + leitura profissional) | `DIARIO_04_04_2026.md` |
...
**02 abr:** **`DIARIO_02_04_2026.md`** — continuidade AEC (âncora somática, placeholders); profissional de destino na narrativa; **uma verdade** para agenda (RPC + card, não grade duplicada no prompt); correção de rota que perdia `location.state`. **§7 (mesmo dia):** auditoria Postgres — fonte de verdade do chat (`patient_medical_records` vs `ai_chat_interactions`); strip RAG na persistência + fallback UI; scripts `AUDIT_*` / `FIX_LEGACY_RAG_*` / `AUDIT_SUPABASE_NOA_ECOSYSTEM_*` / opcional `FIX_DOCUMENTS_UNLINK_*`; clarificação `noa_memories` não ligada à UI; pitch/data room (disciplina de persistência).
...
**02 abril** *(`DIARIO_02_04_2026.md`)*  
*Técnico (manhã / AEC):* `complaintHdaAnchor`, `aecTargetPhysicianDisplayName`, `aecConsultationPhysicianName` no payload; Core PERFIS sem horário fixo; `PatientAppointments` → `/app/clinica/paciente/chat-noa` com state. *Filosófico:* **prompt subordinado ao que o sistema pode provar** — disponibilidade é dado, não folclore do modelo.  
*Técnico (continuação / dados):* `stripPlatformInjectionNoise` em `noaResidentAI`; fallback `ClinicalReports` → `patient_medical_records`; SQL de auditoria e backfill opcional em `database/scripts/`; inventário `documents` / `noa_*` / dual `users`×`pacientes`. *Filosófico:* **governança de persistência** alinhada ao Clinical OS — auditar antes de apagar; RAG da biblioteca (`isLinkedToAI`) separado do prontuário; ausência de linhas em `noa_memories` explicada por wiring, não por “sistema sem memória”.
...
**Como usar:** executar **por fase** (A→I). Para cada linha: marcar `[ ]` → `[x]`, preencher **Data** e **Evidência** (link deploy, print dashboard, ticket). Se algo não aplicar, marcar **N/A** com justificativa. Antes da **Fase I**, cumprir **9.D** (smoke 5/5) salvo exceção documentada.

**Última execução / atualização desta §:** _yyyy-mm-dd — responsável:_

**Mentalidade:** decisões de go-live baseiam-se em **gates** e evidência (Fases A–H, **9.D**, **9.C**), não no número "97%". Até prova em produção: *código maduro em staging + integrações externas por validar*.

### Decisões já tomadas *(não reabrir sem motivo forte)*

- **LLM / Nôa** não executa ato clínico nem substitui decisão do profissional; o **Core** governa o que pode virar ação.  
- **Agenda:** horários válidos vêm de **RPC** (ex. `get_available_slots_v3`); o prompt **não** é fonte de verdade da grade.  
- **RLS** não é contornado por atalho em produção; alteração de políticas só com **auditoria explícita**.  
- **AEC** é **estrutura de dados + fluxo** no produto, não "mais um chat" descontextualizado.
...
| [ ] | **Dualidade `users` / `pacientes`:** risco assumido ou mitigado | Mapa de dados | Decisão registada (**§10.2** + 9.A) |
...
### 9.D Smoke test obrigatório *(antes da Fase I — beta)*

Executar no **mesmo tipo de ambiente** que o beta vai usar (produção ou staging com schema + Edge alinhados).

| # | Teste | Critério de "passou" |
|---|--------|----------------------|
| 1 | Login **paciente** | Sessão estabelecida; sem bloqueio crítico |
| 2 | Fluxo **AEC** até relatório | Resultado refletido em `clinical_reports` (ou contrato equivalente acordado pela equipa) |
| 3 | **Agendamento** | Slots coerentes com **RPC** (`get_available_slots_v3`); reserva consistente |
| 4 | **Vídeo** | WiseCare **ou** WebRTC funciona em rede típica; se P2P falhar, **fallback** documentado e testado |
| 5 | **Chat** (fluxo em teste: P2P ou outro) | **Persistência confirmada** na tabela esperada (ver **§10.2**); sem falha RLS silenciosa |

**Critério global:** **5/5** antes de convidar utilizadores ao beta (**Fase I**). Se algum item falhar: corrigir, ou **N/A** com plano datado (não ignorar).
...
**Legenda:** 🔴 crítico · 🟡 monitorar · 🟢 resolvido *(só marcar 🟢 com evidência datada em **9.A** ou auditoria).*

| | Risco | Nota |
|---|------|------|
| 🔴 | WebRTC **sem TURN** | Falhas em redes corporativas/CGNAT; mitigar com WiseCare ou TURN futuro. |
| 🟡 | **users** vs **pacientes** | Joins/RLS ambíguos; unificação ou regra explícita — mapa **§10.2**; decisão registada. |
| 🟡 | **Financeiro** | Não validado em carga/compliance em produção real. |
| 🟡 | **Custo** Supabase | Edge, Realtime, logs em escala. |
| 🟡 | **Doc vs banco** | Políticas antigas; revalidar antes de due diligence. |
| 🟡 | **Chat / IMRE — fonte única** | Zona ainda difusa (ver **§10.2**, `DIARIO_03_04`); auditoria clínica e analytics dependem de fechar isto. |
| 🟢 | *(reservado)* | Preencher quando um risco for **encerrado** (ex.: RLS revalidado), com data. |
...
### 10.2 Fonte de verdade por domínio *(reduz ambiguidade — atualizar se o schema mudar)*

| Domínio | Fonte de verdade no produto | Observação |
|--------|-----------------------------|------------|
| **Chat clínico / narrativa AEC no prontuário** | `patient_medical_records` (campo de interação / `chat_interaction` conforme migrações) | Não confundir com chat P2P nem com `ai_chat_interactions` vazio por desenho legado — ver §0.1 e `DIARIO_02_04` §7. |
| **Chat P2P (salas)** | `chat_messages` + `chat_rooms` / participantes; legado: `chat_messages_legacy` | Validar RLS e inserts reais — `DIARIO_03_04`, `database/scripts/VERIFY_CHAT_P2P_RLS_READ_ONLY.sql`. |
| **Relatório clínico estruturado** | `clinical_reports` (conteúdo + governança via **Core** / `finalize_assessment`, `tradevision-core`) | UI: `ClinicalReports.tsx`, `PatientAnalytics.tsx`; não inferir relatório só de parsing livre do chat sem contrato. |
| **IMRE / scores derivados** | *A consolidar* — hoje ligados a relatórios e fluxos documentados nos diários | Fechar **uma** fonte explícita (plano `DIARIO_03_04`); até lá, tratar analytics com cautela. |
| **Agenda (slots)** | RPC **`get_available_slots_v3`**; reserva **`book_appointment_atomic`** (e migrações associadas) | Prompt/card só **refletem** a RPC — nunca a substituem. |
| **Vídeo (sessão, pedidos, qualidade)** | `video_call_requests`, fluxo **WiseCare** + logs; WebRTC como caminho alternativo | STUN-only documentado; ver Fase G e **9.C** 🔴. |

*Instrução para IA:* antes de afirmar "onde está o chat" ou "onde está o relatório", cruzar com esta tabela **e** com o schema atual no Supabase.
...
*Fim da edição v2.6. História = Livro Magno + diários + pitch/societário + **pilares humanos (§4.4)**; execução = código + banco + **§9 runbook** (**9.D** + **9.C** + **§10.2**). Para a IA: TL;DR → **§0.1** → **§10.2** → **§9** (marcar checkboxes) → bibliografia §2.*
...
6. **v2.6** — **§10.2** (fonte de verdade por domínio); **9.D** (smoke 5/5 antes do beta); **9.C** com severidade (🔴🟡🟢); bloco **decisões já tomadas** no §9; TL;DR reforça *staging + gates* vs "97%".
...
**Melhoria contínua:** após cada go-live ou deploy relevante, **preencher 9.A** (e mover riscos para 🟢 em **9.C** quando houver evidência) e datar a linha no cabeçalho da §9.

<!-- file-sync 2026-04-02T22:06:37.618Z -->
