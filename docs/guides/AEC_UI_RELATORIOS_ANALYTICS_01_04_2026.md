...
### 1.2 Abertura AEC: “quero avaliação” consumia o passo de apresentação

- **Sintoma:** após “quero fazer avaliação clínica inicial”, a Nôa pedia apresentação; “Pedro aqui” gerava resposta genérica (“Como posso ajudá-lo?”) ou tratava o nome como primeira **queixa**.  
- **Causa:** `skipProcessBecauseStartCmd` exigia `!pedidoAvaliacaoNaMesmaMsg`, logo mensagens com “quero … avaliação” **não** eram ignoradas como meta-pedido e avançavam `INITIAL_GREETING` com texto errado no `patientPresentation`.  
- **Correção (`noaResidentAI.ts` / `processTradeVisionRequest`):** na abertura, **ignorar** processamento clínico quando o texto é só pedido de início (`pedidoAvaliacaoNaMesmaMsg` ou `explicitAssessmentStart`) e não parece apresentação; fixar `nextQuestionHint` na frase de abertura AEC; iniciar fluxo também com **pedido explícito no chat** se ainda não existir `ASSESSMENT_START` do Core; reconhecer nome curto isolado (“Pedro”) como apresentação.
