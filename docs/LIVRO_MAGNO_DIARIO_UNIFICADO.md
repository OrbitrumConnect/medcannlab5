...
**Versão:** 1.0.13  
**Última Atualização:** 2 de Abril de 2026 *(nota 03/04: plano em `DIARIO_03_04_2026.md`)*  
**Responsável:** Antigravity (IA Assistente) & Dr. Ricardo Valença *(entradas 01–02/04: sessão Cursor — AEC, jornada, agenda, auditoria Supabase)*  
...
### 📅 02/04/2026: Uma verdade para a agenda — AEC, profissional de destino e prompt alinhado ao RPC *(sessão Cursor)*

**Contexto:** Dois sistemas conviviam sem ser nomeados: o **motor de vagas** (`get_available_slots_v3` + `professional_availability`) e o **texto institucional** no Core com dias/horários fixos para dois médicos. O paciente podia ouvir da Nôa uma grade e ver **outra** no card. Em paralelo, a AEC precisava **manter a queixa corporal** quando a escolha “o que mais incomoda” era funcional (sono, impacto) e **personalizar** abertura/consentimento com o profissional que o paciente escolheu na vitrine — sem perder a atribuição do **método IMRE** ao Dr. Ricardo Valença.

**Técnico (síntese):**

| Tema | Mudança |
|------|---------|
| HDA / Core | `complaintHdaAnchor`; instruções sem `[queixa]` literal; menos loop em `COMPLAINT_DETAILS` |
| Jornada vitrine → chat | `aecTargetProfessional` no hook; `startAssessment(..., targetDoc)`; payload `aecConsultationPhysicianName`; rota canônica do chat paciente preserva `location.state` |
| Core “secretária” | Bloco PERFIS: especialidade e papel **sim**; horários **não** no prompt — remeter ao card / `[TRIGGER_SCHEDULING]` |

**Documentação:** `DIARIO_02_04_2026.md` §1–6; `docs/TIMELINE_LIVRO_MAGNO_DIARIOS_CONSOLIDADO.md` (§3, §7.2, §0.1).

**Filosófico:** O LLM é **intérprete** da instituição, não **fonte paralela** de calendário. Onde há dado estruturado e RPC determinístico, a fala deve **ceder** — ou o produto treina o usuário a desconfiar da própria interface.

---

### 📅 02/04/2026 (mesmo dia): Auditoria Supabase — prontuário, Nôa e biblioteca *(sessão Cursor)*

**Contexto:** Operação e investidor pedem **prova reversível** de onde vivem os dados (chat vs `ai_chat_interactions`), de **higiene** na persistência (sem vazar blocos RAG internos no JSON clínico) e de **curadoria** da biblioteca RAG (`documents`). Erros comuns no SQL Editor (`:placeholders`, nomes de colunas inexistentes) atrasavam a auditoria.

**Técnico (síntese):**

| Tema | Mudança |
|------|---------|
| Gravação chat | `noaResidentAI`: `stripPlatformInjectionNoise` em `user_message` / `ai_response` ao persistir em `patient_medical_records` |
| UI relatórios | `ClinicalReports.tsx`: fallback de conversa para `patient_medical_records` (`chat_interaction`) quando `ai_chat_interactions` está vazio, com strip na resposta |
| SQL repo | `AUDIT_CHAT_RAG_CONTEXTO_…`, `FIX_LEGACY_RAG_CONTEXTO_2_ROWS_…`, `FIX_LEGACY_RAG_CONTEXTO_ALL_…`, `AUDIT_SUPABASE_NOA_ECOSYSTEM_READ_ONLY.sql`, `FIX_DOCUMENTS_UNLINK_DEMO_TITLES_FROM_AI.sql` (último opcional) |
| Pitch / data room | Item de **disciplina de persistência** e auditoria amostral no kit (`pitch_deck_medcannlab.md`, `pitch_unificado_completo.md`) |

**Documentação:** `DIARIO_02_04_2026.md` §7; scripts em `database/scripts/`.

**Filosófico:** **Clinical OS** inclui **governança de dados**: leitura antes de escrita, backfill documentado, separação clara entre **prontuário**, **tabela de chat legada** (`ai_chat_interactions` quando usada), **biblioteca** (`documents.isLinkedToAI`) e **`noa_memories`** (hoje sem escrita pela UI — não confundir ausência de linhas com “falha do sistema”).

---

### 📅 03/04/2026 *(plano — executar e depois selar no diário)*

**Documentação:** `DIARIO_03_04_2026.md` — depois do inventário Lovable e dos scripts SQL: **validar** `chat_messages` vs `chat_messages_legacy` (RLS / fluxo real), **documentar** onde IMRE/AEC persistem hoje sem assumir `imre_assessments` vazio = bug; no máximo **uma** melhoria conectada (ex. consumir uma view no dashboard) com rollback simples; explícito o que **não** fazer no mesmo dia (migração massiva, RLS aberto, dados legados apagados).
