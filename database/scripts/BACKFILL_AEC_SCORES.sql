-- ============================================================
-- 🧬 BACKFILL DE SCORES AEC (clinical_reports.content.scores)
-- ============================================================
-- Versão corrigida após review do GPT:
--   ✅ STABLE (não IMMUTABLE — sin/cos não são imutáveis)
--   ✅ WHERE corretamente parentizado (sem precedência ambígua)
--   ✅ Cast blindado por regex (^\d+$) — não quebra com texto
--   ✅ Backfill em batch de 500 linhas (sem lock pesado)
--
-- Espelha 1:1 a lógica de src/lib/clinicalScoreCalculator.ts
-- 8 sinais ponderados:
--   queixa_principal (15)  | lista_indiciaria (20)  | desenvolvimento (20)
--   habitos (15)           | historia_patologica (10) | familiar (10)
--   perguntas_objetivas (10) | consenso (5)
--
...
DECLARE
  ...
  total_weight  int := 0;
  signals jsonb := '[]'::jsonb;
  v_text text; v_arr jsonb; v_obj jsonb; v_count int;
  s_queixa int := 0; s_lista int := 0; s_dev int := 0; s_habitos int := 0;
  s_hpp int := 0; s_fam int := 0; s_perg int := 0; s_consenso int := 0;
  fpiora jsonb; fmelhora jsonb; sintomas jsonb; materno jsonb; paterno jsonb;
  raw_score int; clinical_score int; has_consent boolean;
  base int; variance int; adherence int;
  habitos_count int; bonus int; qol int;
  melhora_count int; piora_count int; symptoms int;
  filled int; confidence text; has_any boolean;
BEGIN
  IF content IS NULL OR jsonb_typeof(content) <> 'object' THEN
    RETURN jsonb_build_object('clinical_score',0,'treatment_adherence',0,'quality_of_life',0,'symptom_improvement',0,'calculated',false,'score_confidence','low','source_signals','[]'::jsonb);
  END IF;
  ...
  total_signals := total_signals + s_lista; total_weight := total_weight + 20;
  ...
  END IF;
  total_signals := total_signals + s_dev; total_weight := total_weight + 20;
  ...
  IF jsonb_typeof(v_arr) <> 'array' THEN v_arr := '[]'::jsonb; END IF;
  v_count := jsonb_array_length(v_arr);
  s_habitos := LEAST(15, v_count * 3);
  total_signals := total_signals + s_habitos; total_weight := total_weight + 15;
  ...
  total_signals := total_signals + s_hpp; total_weight := total_weight + 10;
  ...
  END IF;
  total_signals := total_signals + s_fam; total_weight := total_weight + 10;
  ...
  total_signals := total_signals + s_perg; total_weight := total_weight + 10;
  ...
  has_consent := COALESCE((content->'consenso'->>'aceito')::boolean, false);
  ...
  v_arr := COALESCE(content->'habitos_vida', content->'lifestyle', '[]'::jsonb);
  habitos_count := CASE WHEN jsonb_typeof(v_arr) = 'array' THEN jsonb_array_length(v_arr) ELSE 0 END;
  ...
  SELECT count(*) INTO filled FROM jsonb_array_elements(signals) s WHERE (s->>'signal_value')::int > 0;
  ...
  RETURN jsonb_build_object(
    ...
    'calculated', has_any, 'score_confidence', confidence, 'source_signals', signals
  );
END;
$$;

-- Backfill em batch de 500 linhas
DO $$
DECLARE affected int;
BEGIN
  LOOP
    WITH targets AS (
      SELECT id FROM public.clinical_reports
    ...
    SET content = jsonb_set(COALESCE(cr.content, '{}'::jsonb), '{scores}', public.compute_aec_scores(cr.content), true)
    FROM targets t WHERE cr.id = t.id;
...
-- Verificação rápida
SELECT
  count(*) FILTER (WHERE (content->'scores'->>'clinical_score')::int > 0) AS com_score,
  count(*) FILTER (WHERE (content->'scores'->>'clinical_score') IS NULL OR (content->'scores'->>'clinical_score')::int = 0) AS sem_score,
  count(*) AS total
