-- =====================================================================
-- BACKFILL DE SCORES CLÍNICOS AEC — v2 (endurecido pós-review)
-- =====================================================================
-- Como rodar:
--   1. Abra: https://supabase.com/dashboard/project/itdjkfubfzmvmuxxjoae/sql/new
--   2. Cole este arquivo inteiro
--   3. Execute (Run)
--   4. Verifique: SELECT count(*) FILTER (WHERE (content->'scores'->>'clinical_score')::int > 0) FROM public.clinical_reports;
--
-- Melhorias vs v1:
--   ✔ Aceita variações de consenso ("true"/"1"/"sim"/"yes"/"t"/"y")
--   ✔ Conta sinais sem reparse de jsonb_array_elements
--   ✔ Loop com teto de 10.000 iterações (cinto + suspensório)
--   ✔ search_path com pg_temp para isolamento
--   ✔ Coeficientes sin/cos mantidos idênticos ao clinicalScoreCalculator.ts
--     do front (Verdade Atômica — mesmo paciente, mesmo score em qualquer lugar)
-- =====================================================================
...
BEGIN
  ...
  -- Sinal 1: Queixa Principal (peso 15)
  ...
  -- Sinal 2: Lista Indiciária (peso 20)
  ...
  -- Sinal 4: Hábitos de Vida (peso 15)
  ...
  -- Sinal 6: História Familiar (peso 10)
  ...
  -- Sinal 7: Perguntas Objetivas (peso 10)
  ...
  -- Sinal 8: Consenso (peso 5) — aceita variações comuns
...
-- Backfill em batch com teto de segurança
DO $$
DECLARE
  affected int;
  iterations int := 0;
BEGIN
  LOOP
    ...
    SET content = jsonb_set(
      COALESCE(cr.content, '{}'::jsonb),
      '{scores}',
      public.compute_aec_scores(cr.content),
      true
    )
    FROM targets t
    WHERE cr.id = t.id;
    ...
    EXIT WHEN affected = 0 OR iterations > 10000;
  END LOOP;
END $$;
