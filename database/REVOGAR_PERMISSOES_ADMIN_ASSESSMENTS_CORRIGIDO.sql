-- =====================================================
-- REVOGAR PERMISSÕES IRRESTRITAS DE ADMIN - VERSÃO CORRIGIDA
-- MedCannLab 3.0 - Segurança e Privacidade
-- =====================================================
-- ⚠️ IMPORTANTE: Este script remove TODAS as políticas que dão
-- acesso irrestrito aos admins e cria políticas restritivas.
-- =====================================================

-- 1. REMOVER TODAS AS POLÍTICAS PROBLEMÁTICAS
-- =====================================================

-- Remover TODAS as políticas de admin que dão acesso irrestrito
...
-- Remover políticas antigas de admin que possam existir
...
-- =====================================================
SELECT 
  'Políticas restantes após remoção:' as info,
  policyname,
  cmd
FROM pg_policies
...
-- Política 1: Pacientes podem ver suas próprias avaliações
...
-- Política 4: Profissionais podem ver avaliações de seus pacientes
...
-- Política 5: Profissionais podem inserir avaliações para seus pacientes
...
-- Política 6: Profissionais podem atualizar avaliações de seus pacientes
...
-- Política 7: Admins podem ver APENAS avaliações onde são o médico responsável
...
-- Política 8: Admins podem inserir avaliações APENAS como médico responsável
...
-- Política 9: Admins podem atualizar avaliações APENAS onde são o médico responsável
...
-- Política 10: Admins podem ver avaliações compartilhadas explicitamente
-- (Se houver tabela assessment_sharing)
...
CREATE POLICY "Admins can view shared assessments with consent"
  ...
  USING (
    ...
    AND EXISTS (
      -- Verificar se há compartilhamento explícito com consentimento
      SELECT 1 FROM assessment_sharing
      WHERE assessment_sharing.assessment_id = clinical_assessments.id
      AND (
        -- Compartilhado com Ricardo Valença
        (
          assessment_sharing.shared_with_ricardo_valenca = TRUE 
          AND auth.uid() = (SELECT id FROM users WHERE email IN ('rrvalenca@gmail.com', 'rrvlenca@gmail.com', 'profrvalenca@gmail.com') LIMIT 1)
        )
        OR
        -- Compartilhado com Eduardo Faveret
        (
...
SELECT 
  ...
  cmd as "Comando",
  CASE 
    WHEN qual LIKE '%doctor_id = auth.uid()%' THEN '✅ Restritiva - Apenas médico responsável'
...
ORDER BY 
  CASE 
    WHEN qual LIKE '%doctor_id = auth.uid()%' THEN 1
    WHEN qual LIKE '%patient_id = auth.uid()%' THEN 2
    WHEN qual LIKE '%shared%' THEN 3
    ELSE 4
  END,
  policyname;

-- 5. TESTE DE SEGURANÇA CORRIGIDO
-- =====================================================
-- Este teste deve mostrar que admin NÃO vê todas as avaliações
SELECT 
  'Teste de Segurança Corrigido' as teste,
  COUNT(*) as total_assessments_no_sistema,
  COUNT(*) FILTER (WHERE doctor_id = auth.uid()) as assessments_do_admin_como_medico,
  COUNT(*) FILTER (WHERE patient_id = auth.uid()) as assessments_do_admin_como_paciente,
  COUNT(*) FILTER (WHERE doctor_id != auth.uid() AND patient_id != auth.uid()) as assessments_nao_acessiveis,
  CASE 
    WHEN COUNT(*) FILTER (WHERE doctor_id != auth.uid() AND patient_id != auth.uid()) = 0 
      AND COUNT(*) FILTER (WHERE doctor_id = auth.uid()) < COUNT(*)
    THEN '✅ SEGURO - Admin só vê suas avaliações'
    WHEN COUNT(*) FILTER (WHERE doctor_id != auth.uid() AND patient_id != auth.uid()) = COUNT(*)
    THEN '✅ SEGURO - Admin não vê nenhuma avaliação de outros'
    ELSE '⚠️ VERIFICAR - Pode haver problema'
  END as resultado_seguranca
FROM clinical_assessments;

-- 6. VERIFICAR SE HÁ AVALIAÇÕES SEM DOCTOR_ID
-- =====================================================
