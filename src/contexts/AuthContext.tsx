...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUser = async (authUser: any) => {
    ...
    try {
      ...
      if (!userError && userData) {
        ...
        if ((userData as any).trial_ends_at) {
          dbTrialEndsAt = (userData as any).trial_ends_at
        }
        if ((userData as any).consent_accepted_at) {
          dbConsentAcceptedAt = (userData as any).consent_accepted_at
        }
    ...
    // Não usar isValidUserType no nome: normalizeUserType usa includes('paciente'), etc., e nomes como
    // "Pedro Paciente" eram tratados erroneamente como rótulo de papel.
    ...
    // Trial: ler do banco (coluna trial_ends_at) — fallback para cálculo local
    const createdAt = authUser.created_at ? new Date(authUser.created_at) : null
    const trialEndsAt: Date | undefined = dbTrialEndsAt
      ? new Date(dbTrialEndsAt)
      : (createdAt ? new Date(createdAt.getTime() + 3 * 24 * 60 * 60 * 1000) : undefined)
    ...
    const debugUser: User = {
      ...
      onboarding_completed_at: dbOnboardingCompletedAt
    }
