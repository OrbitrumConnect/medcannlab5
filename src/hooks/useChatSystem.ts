...
export interface ChatSystemState {
  ...
  isOnline: boolean
  sendMessage: (roomId: string, senderId: string, content: string, messageType?: string, fileUrl?: string) => Promise<void>
...
export const useChatSystem = (activeRoomId?: string, options?: UseChatSystemOptions): ChatSystemState => {
  ...
  const sendMessage = useCallback(
    async (roomId: string, senderId: string, content: string, messageType: string = 'text', fileUrl?: string) => {
      if (!enabled) return

      const trimmed = content.trim()
      if (!trimmed && !fileUrl) return

      const encryptedContent = trimmed ? await encryptMessage(trimmed) : ''

      const { data: sessionData } = await supabase.auth.getSession()
      const authUid = sessionData.session?.user?.id
      if (authUid && senderId !== authUid) {
        console.warn(
          '[useChatSystem] senderId difere de auth.uid(); a policy RLS de INSERT em chat_messages pode rejeitar.',
          { senderId, authUid, roomId }
        )
      }
      ...
      if (error) {
        console.error('[useChatSystem] Falha ao inserir chat_messages:', {
