...
interface UseWiseCareRoomReturn {
    ...
    // Ações
    createAndJoin: (options?: Partial<CreateSessionOptions>) => Promise<VideoSession | void>;
    joinExistingSession: (existingSession: VideoSession) => Promise<void>;
    getSession: (appointmentId: string) => Promise<VideoSession | null>;
...
export function useWiseCareRoom(options: UseWiseCareRoomOptions = {}): UseWiseCareRoomReturn {
    ...
    // Entrar em uma sessão já existente
    const joinExistingSession = useCallback(async (existingSession: VideoSession) => {
        if (!providerRef.current) {
            setError('Provider not initialized');
            return;
        }
        ...
        try {
            await providerRef.current.joinSession(existingSession, {
                containerId,
                startWithAudioMuted,
                startWithVideoMuted,
                width: '100%',
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to join existing WiseCare session';
            setError(errorMessage);
            setIsConnecting(false);
            onError?.(err instanceof Error ? err : new Error(errorMessage));
        }
    }, [containerId, startWithAudioMuted, startWithVideoMuted, onConnect, onDisconnect, onError]);
    ...
    // Buscar sessão ativa
    const getSessionFn = useCallback(async (appId: string): Promise<VideoSession | null> => {
        if (!providerRef.current) return null;
        return providerRef.current.getSession(appId);
    }, []);
