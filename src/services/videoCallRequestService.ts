...
class VideoCallRequestService {
  ...
  async getPendingRequests(): Promise<VideoCallRequest[]> {
    // 🛑 ANTI-LOOP de REDE: Se o PWA perdeu conexão, interromper polling sumariamente.
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      return []
    }

    // Usar getSession (local, sem HTTP) em vez de getUser (HTTP request) para evitar auth lock flood
  ...
  async expireOldRequests(): Promise<void> {
    // 🛑 ANTI-LOOP de REDE: Salvar bateria e rede se offline
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      return
    }
  ...
  subscribeToRequests(
    ...
    // Supabase Realtime NÃO suporta OR em filtros.
    // Criamos dois canais separados: um para INSERT (recipient) e outro para UPDATE (requester).
    const channelRecipient = supabase
      .channel(`vcr-recipient:${userId}`)
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
