...
export class KnowledgeBaseIntegration {
  /**
   * Buscar TODOS os documentos da base de conhecimento (sem limite)
   * Usado pela IA Residente para ter acesso completo à base de conhecimento
   */
  static async getAllDocuments(): Promise<KnowledgeDocument[]> {
    try {
      // Buscar TODOS os documentos sem limite
      // Supabase tem limite padrão de 1000 registros, mas podemos aumentar se necessário
      ...
      console.log(`📚 Total de documentos carregados para IA: ${data?.length || 0}`)
      return data || []
    } catch (error) {
  ...
  static async semanticSearch(
    ...
    try {
      ...
      // Busca por texto - otimizada para evitar queries muito longas
      const searchTerm = query.toLowerCase().trim()
      
      // Truncar mensagens muito longas (>200 caracteres) para evitar queries gigantes
      const truncatedQuery = searchTerm.length > 200 
        ? searchTerm.substring(0, 200) 
        : searchTerm
      
      // Extrair palavras-chave (limitado a 10 palavras para evitar queries muito longas)
      ...
      // Busca exata no título (para nomes de arquivo) - apenas se query não for muito longa
      if (truncatedQuery.length > 0 && truncatedQuery.length <= 100) {
        searchConditions.push(`title.ilike.%${truncatedQuery}%`)
        searchConditions.push(`summary.ilike.%${truncatedQuery}%`)
        
        // Se houver extensão de arquivo, buscar também
        if (truncatedQuery.includes('.')) {
          searchConditions.push(`title.ilike.%${truncatedQuery.replace(/\./g, '%.')}%`)
        }
      }
      
      // Busca por palavras individuais (limitado para evitar queries muito longas)
      // Máximo de 15 condições para evitar erro de parsing no Supabase
      const maxWordConditions = Math.min(searchTerms.length, 6) // 6 palavras = 12 condições (title + summary)
      for (let i = 0; i < maxWordConditions; i++) {
        const term = searchTerms[i]
        searchConditions.push(`title.ilike.%${term}%`)
        searchConditions.push(`summary.ilike.%${term}%`)
      }
      
      // Validar que não temos mais de 50 condições (limite seguro)
      if (searchConditions.length > 0 && searchConditions.length <= 50) {
        queryBuilder = queryBuilder.or(searchConditions.join(','))
      } else if (searchConditions.length > 50) {
        // Se exceder, usar apenas as primeiras condições mais importantes
        console.warn('⚠️ Query muito longa, usando apenas primeiras 25 condições')
        queryBuilder = queryBuilder.or(searchConditions.slice(0, 25).join(','))
      }
  ...
  static async registerDocumentUsage(
    ...
    try {
      // Primeiro, buscar o documento atual para obter o valor atual de downloads
      ...
      if (fetchError) {
        console.warn('⚠️ Erro ao buscar documento para incrementar downloads:', fetchError)
      }
