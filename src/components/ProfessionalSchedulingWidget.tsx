...
interface ProfessionalSchedulingWidgetProps {
  className?: string
  patientId?: string
}
...
const ProfessionalSchedulingWidget: React.FC<ProfessionalSchedulingWidgetProps> = ({ className = '', patientId }) => {
  ...
  const loadAvailabilityRules = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        setAvailabilityRules(data.map((r: any) => ({
          id: r.id,
        ...
        // Defaults: Seg-Sex, 08:00-18:00
        setAvailabilityRules([1, 2, 3, 4, 5].map(d => ({
          day_of_week: d, start_time: '08:00', end_time: '18:00', slot_duration: 60, is_active: true
        })))
      }
    } catch (err) {
  ...
  const toggleDay = (dow: number) => {
    setAvailabilityRules(prev => {
      const exists = prev.find(r => r.day_of_week === dow)
      if (exists) {
        return prev.map(r => r.day_of_week === dow ? { ...r, is_active: !r.is_active } : r)
      }
      return [...prev, { day_of_week: dow, start_time: '08:00', end_time: '18:00', slot_duration: 60, is_active: true }]
    })
  }

  const updateRule = (dow: number, field: keyof AvailabilityRule, value: any) => {
    setAvailabilityRules(prev => prev.map(r => r.day_of_week === dow ? { ...r, [field]: value } : r))
  }
  ...
  return (
    <div className={`space-y-4 ${className}`}>
          ...
          <div className="bg-slate-800/40 backdrop-blur-sm border border-slate-600/40 rounded-xl p-5 shadow-lg">
            <div className="flex items-center justify-between mb-5">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-lg transition-all shadow-lg shadow-emerald-500/20 disabled:opacity-50"
              >
            ...
            {showSuccess && activeTab === 'config' && (
              <div className="mt-4 flex items-center gap-2 text-emerald-400 text-sm font-semibold animate-in fade-in duration-300">
      ...
      {isDayModalOpen && selectedDay && (() => {
        ...
        // Gerar horários livres a partir da config de disponibilidade
        const dayOfWeek = dayDate.getDay()
        const rule = availabilityRules.find(r => r.day_of_week === dayOfWeek && r.is_active)
        const workSlots: string[] = []
        if (rule) {
          const [sh, sm] = rule.start_time.split(':').map(Number)
          const [eh, em] = rule.end_time.split(':').map(Number)
          const startMin = sh * 60 + (sm || 0)
          const endMin = eh * 60 + (em || 0)
          for (let m = startMin; m < endMin; m += rule.slot_duration) {
            const hh = String(Math.floor(m / 60)).padStart(2, '0')
            const mm = String(m % 60).padStart(2, '0')
            workSlots.push(`${hh}:${mm}`)
          }
        }
...
export default ProfessionalSchedulingWidget
