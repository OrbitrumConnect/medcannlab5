...
const getPositionClasses = (
  position: NoaConversationalInterfaceProps["position"],
) => {
  switch (position) {
    case "bottom-left":
...
const NoaConversationalInterface = React.forwardRef<
  HTMLDivElement,
...
>(
  (
    ...
    const [uploadUserType, setUploadUserType] = useState<string[]>([
      "professional",
      "student",
    ]);
    // Estados para gravação de consulta
    const [isRecordingConsultation, setIsRecordingConsultation] =
      useState(false);
    ...
    const isFlushingRef = useRef(false); // Lock para evitar double-send no onend/flush
    ...
    const openInlineDocument = useCallback(
      async (documentId: string) => {
        ...
        try {
          const { data, error } = await supabase
            .from("documents")
            .select(
              "id, title, summary, content, file_url, file_type, category, created_at, updated_at",
            )
            .eq("id", id)
            .maybeSingle();
          ...
          setInlineDocError(
            "Não foi possível carregar o documento agora. Verifique permissões/RLS.",
          );
        } finally {
    ...
    // Listener: Executar comandos da Nôa (Navegação, Documentos, Prescrições, etc)
    useEffect(() => {
      const handleNoaCommand = (event: Event) => {
        ...
        console.log("📡 [NoaConversationalInterface] Recebido comando via Evento:", detail.type, detail);

        switch (detail.type) {
          case "show-document-inline": {
            const docId = (
          ...
          case "navigate-section": {
            ...
            // Mapeamento determinístico de seções para rotas e parâmetros
            if (target === "terminal-clinico" || target === "atendimento") {
              // No dashboard unificado, usamos query params para trocar a seção
              navigate(`/app/clinica/profissional/dashboard?section=${target}`);
            } else if (target === "pacientes") {
              navigate("/app/clinica/profissional/pacientes");
            } else if (target === "relatorios-clinicos") {
              navigate("/app/clinica/profissional/relatorios");
            } else if (target === "chat-profissionais") {
              navigate("/app/clinica/profissional/chat-profissionais");
            } else if (target === "agendamentos") {
          ...
          case "show-prescription": {
            // Documentação diz que deve abrir prescrições
            navigate("/app/clinica/prescricoes");
            break;
          }

          case "filter-patients": {
            const filter = detail.payload?.filter || detail.target || "active";
            navigate(
              `/app/clinica/profissional/dashboard?section=terminal-clinico&filter=${filter}`,
            );
            break;
          }
          ...
          case "open-document": {
            if (detail.target) {
              // Se for uma URL externa ou rota específica
              if (
                detail.target.startsWith("http") ||
                detail.target.startsWith("/")
              ) {
                window.location.href = detail.target;
              } else {
                openInlineDocument(detail.target);
              }
            }
            break;
          }
          ...
          default:
            console.warn("⚠️ Tipo de comando não reconhecido:", detail.type);
        }
      };
      ...
      return () =>
        window.removeEventListener(
          "noaCommand",
          handleNoaCommand as EventListener,
        );
    }, [openInlineDocument, navigate]);
    ...
    useEffect(() => {
      ...
      if (wasSpeakingRef.current && !isSpeaking) {
        ...
        console.log(
          "🔇 IA terminou de falar, registrando timestamp para evitar eco",
        );
      }
    ...
    useEffect(() => {
      ...
      const hasActiveSimulation = messages.some((msg) => {
        ...
        return (
          metadata?.simulationActive === true &&
      ...
      const hasActiveAssessment = messages.some((msg) => {
        const metadata = msg.metadata as Record<string, any> | undefined;
        const nestedMetadata = metadata?.metadata as
          | Record<string, any>
          | undefined;

        return (
          metadata?.assessmentActive === true ||
          nestedMetadata?.assessmentActive === true ||
          metadata?.type === "assessment" ||
          nestedMetadata?.type === "assessment" ||
          (metadata?.intent === "IMRE_ANALYSIS" &&
            nestedMetadata?.assessmentActive !== false)
        );
      });

      hasActiveSimulationRef.current =
        hasActiveSimulation || hasActiveAssessment;

      if (hasActiveSimulation) {
        console.log(
          "🎭 Simulação ativa detectada - microfone terá prioridade máxima",
        );
      }
      if (hasActiveAssessment) {
        console.log(
          "📋 Avaliação clínica ativa detectada - microfone terá prioridade máxima",
        );
      }
      ...
      if (hasActiveAssessment || hasActiveSimulation) {
        ...
        if (lastMessage) {
          console.log("🔍 Metadata da última mensagem:", {
            ...
            nestedMetadata: (lastMessage.metadata as Record<string, any>)
              ?.metadata,
          });
        }
      }
    ...
    useEffect(() => {
      ...
      const shouldBeOpen =
        (hideButton && position === "inline") || contextIsOpen;
      console.log("🔍 NoaConversationalInterface - Atualizando isOpen:", {
        shouldBeOpen,
        hideButton,
        contextIsOpen,
        position,
    ...
    const stopListening = useCallback(() => {
      ...
      if (recognitionRef.current && recognitionRef.current.recognition) {
        try {
          ...
          console.log(
            "🔍 Estado do reconhecimento antes de parar:",
            currentState,
          );
        } catch (e) {
          console.warn(
            "⚠️ Não foi possível verificar estado antes de parar:",
            e,
          );
        }
      }
      ...
      async (force: boolean = false): Promise<void> => {
        ...
        if (!force) {
          ...
          if (isStartingRef.current) {
            console.log(
              "⏳ Já há uma inicialização em andamento, ignorando chamada duplicada",
            );
            return;
          }
          ...
          if (recognitionRef.current && recognitionRef.current.recognition) {
            try {
              ...
              if (currentState === "listening" || currentState === "starting") {
                console.log(
                  "✅ Reconhecimento já está ativo, não reiniciar:",
                  currentState,
                );
                return;
              }
        ...
        if (
          ...
          alert(
            "Reconhecimento de voz não está disponível no seu navegador. Por favor, use o campo de texto para digitar sua mensagem.",
          );
          return;
        }
        ...
        if (recognitionRef.current && recognitionRef.current.recognition) {
          try {
            ...
            if (currentState === "listening" || currentState === "starting") {
              console.log(
                "🔄 Parando reconhecimento anterior antes de iniciar novo",
              );
              recognitionRef.current.recognition.stop();
              recognitionRef.current.stopped = true;
            }
            ...
            console.log(
              "ℹ️ Não foi possível verificar estado do reconhecimento, continuando...",
            );
          }
        }
        ...
        try {
          ...
          if (
            ...
            alert(
              "Permissão de microfone negada. Por favor, permita o acesso ao microfone nas configurações do navegador e tente novamente.",
            );
            return;
          } else if (
            ...
            alert(
              "Nenhum microfone encontrado. Verifique se o dispositivo está conectado e tente novamente.",
            );
            return;
          } else {
            console.warn(
              "⚠️ Aviso ao verificar permissão, mas continuando...",
              error,
            );
          }
        }
        ...
        const flush = () => {
          ...
          if (text.length > 0) {
            ...
            try {
              ...
              isFlushingRef.current = false; // Reset in case of failure
            }
          } else {
        ...
        const scheduleFlush = () => {
          ...
          handle.timer = window.setTimeout(() => {
            console.log(
              "⏰ Timer de flush() disparado. Buffer:",
              handle.buffer,
            );
            flush();
          }, 1500);
        ...
        recognition.onresult = (event: any) => {
          console.log(
            "🎤 onresult disparado! Número de resultados:",
          ...
          for (let i = event.resultIndex; i < event.results.length; i++) {
            ...
            if (cleanedTranscript.length > 0) {
              if (result.isFinal) {
                ...
                console.log(
                  "🎤 Texto capturado (intermediário, limpo):",
                  cleanedTranscript,
                  "(original:",
                  rawTranscript + ")",
                );
                ...
                console.log(
                  "📝 Input atualizado com texto intermediário:",
                  currentIntermediate,
                );
              }
            }
          ...
          if (allFinalTexts.length > 0) {
            ...
            console.log(
              "✅ MICROFONE FUNCIONANDO! Texto final capturado e validado:",
              finalText,
            );
            ...
            console.log(
              "✅ MICROFONE FUNCIONANDO! Texto intermediário:",
              intermediateText,
            );
            ...
            console.log(
              "📝 Input e buffer atualizados com texto intermediário:",
              intermediateText,
            );
        ...
        recognition.onerror = (event: any) => {
          ...
          if (event.error === "aborted") {
            ...
            console.log(
              "ℹ️ Reconhecimento abortado - não reiniciando automaticamente",
            );
            return;
          }
          ...
          console.error(
            "❌ Erro crítico no reconhecimento de voz:",
            event.error,
          );
          ...
          if (
            ...
            alert(
              "Não foi possível acessar o microfone. Verifique se o dispositivo está conectado e tente novamente.",
            );
        ...
        recognition.onstart = () => {
          ...
          if (recognitionRef.current === handle) {
            // Resetar lock de envio quando o microfone RE-INICIA (onstart)
            ...
            console.log(
              "✅ Estado atualizado via onstart - usuário ativou microfone",
            );
          }
        };
        ...
        recognition.onend = () => {
          ...
          // Enviar texto capturado se houver e se não estivermos já fazendo flush
          if (!isFlushingRef.current && handle.buffer.trim().length > 0) {
            console.log("📤 Enviando texto capturado no onend:", handle.buffer);
            if (handle.timer) {
              window.clearTimeout(handle.timer);
              handle.timer = undefined;
            }
          ...
          console.log(
            "✅ Microfone parou - controle manual: clique no botão para ligar novamente",
          );
          setIsListening(false);
          isListeningRef.current = false;
          recognitionRef.current = null;
          isFlushingRef.current = false; // Resetar lock ao final real
        };
        ...
        try {
          ...
          console.log(
            "🎤 Comando start() enviado ao reconhecimento - aguardando confirmação...",
          );
          ...
          const fallbackTimer = setTimeout(() => {
            if (recognitionRef.current === handle && !isListeningRef.current) {
              console.log(
                "⚠️ onstart não disparou após 1s, verificando estado real do reconhecimento...",
              );
              try {
                ...
                if (state === "listening" || state === "starting") {
                  ...
                  isStartingRef.current = false; // Liberar lock
                  console.log(
                    "✅ Estado atualizado via fallback - reconhecimento confirmado como ativo",
                  );
                  ...
                  if (hasActiveSimulation) {
                    console.log(
                      "🔄 Avaliação/Simulação ativa - tentando reiniciar reconhecimento...",
                    );
                    setTimeout(() => {
                      if (
                        recognitionRef.current === handle &&
                        ...
                        try {
                          recognition.start();
                          console.log(
                            "🔄 Tentativa de reinício do reconhecimento",
                          );
                        } catch (e) {
                    ...
                    console.error(
                      "❌ Reconhecimento NÃO iniciou! Estado:",
                      state,
                    );
                    setIsListening(false);
                    isListeningRef.current = false;
                    ...
                    console.warn(
                      "⚠️ Não foi possível iniciar o microfone. Estado:",
                      state,
                    );
                  }
                } else {
                  ...
                  console.warn(
                    "⚠️ Estado inesperado do reconhecimento:",
                    state,
                  );
                  isStartingRef.current = false; // Liberar lock
                }
                ...
                console.error(
                  "❌ Erro ao verificar estado do reconhecimento:",
                  e,
                );
    ...
    useEffect(() => {
      ...
      if (justFinishedSpeaking) {
        console.log(
          "🔊 [BACKUP] IA terminou de falar, preparando para ligar microfone automaticamente...",
          {
            wasSpeaking,
            isSpeaking,
            isProcessing,
        ...
        const autoStartTimer = setTimeout(() => {
          ...
          if (recognitionRef.current && recognitionRef.current.recognition) {
            try {
              const currentState = recognitionRef.current.recognition.state;
              recognitionActuallyActive =
                currentState === "listening" || currentState === "starting";
          ...
          if (!isStartingRef.current) {
            console.log(
              "🎤✅ [BACKUP] Ligando microfone automaticamente após IA terminar de falar",
            );
              ...
              setTimeout(() => {
                if (!isStartingRef.current && isOpen) {
                  startListening().catch((err) => {
                    console.error(
                      "❌ [BACKUP] Erro ao tentar ligar microfone novamente:",
                      err,
                    );
                  });
                }
    ...
    useEffect(() => {
      if (!isOpen) {
        ...
        if (hasActiveSimulation) {
          console.log(
            "🎭 [SIMULAÇÃO] Chat fechado mas simulação ativa - mantendo microfone",
          );
          return;
        }
    ...
    const handleImmediateListeningRef = useRef<((event: Event) => void) | null>(
      null,
    );
    ...
    useEffect(() => {
      ...
      handleImmediateListeningRef.current = (event: Event) => {
        ...
        if (isRecordingConsultation || showPatientSelector) {
          console.log(
            "⚠️ Não ligando microfone: gravando consulta ou mostrando seletor",
          );
          return;
        }
        ...
        const triggerListening = () => {
          ...
          console.log(
            "🎤✅ Ligando microfone via evento noaImmediateListeningRequest",
          );
          startListening().catch((error) => {
    ...
    const toggleListening = useCallback(async () => {
      ...
      if (isProcessing || isRecordingConsultation) {
        console.log(
          "⚠️ Não é possível usar microfone: processando ou gravando consulta",
        );
        return;
      }
      ...
      if (isListening || isListeningRef.current) {
        ...
        if (isOpen) {
          console.log(
            "⚠️ [CHAT ABERTO] Não é possível parar microfone manualmente - ele deve permanecer ligado",
          );
          return;
        }
        if (hasActiveSimulation) {
          console.log(
            "⚠️ [AVALIAÇÃO ATIVA] Não é possível parar microfone manualmente durante avaliação",
          );
          return;
        }
        ...
        console.log(
          "▶️ Iniciando microfone manualmente",
          hasActiveSimulation ? "[AVALIAÇÃO ATIVA]" : "",
        );
        ...
        try {
          ...
          setTimeout(async () => {
            try {
              ...
              alert(
                "Não foi possível iniciar o microfone. Verifique as permissões e tente novamente.",
              );
            }
          }, 1000);
        }
    ...
    const loadPatients = useCallback(async () => {
      ...
      try {
        ...
        const patientIds = [
          ...new Set(
            assessments?.map((a: any) => a.patient_id).filter(Boolean) || [],
          ),
        ];
    ...
    const handleStartConsultationRecording = useCallback(async () => {
      ...
      if (userType !== "profissional" && userType !== "admin") {
        sendMessage("Apenas profissionais podem gravar consultas.", {
          preferVoice: false,
        });
        return;
      }
      ...
      if (!selectedPatientId) {
        ...
        sendMessage(
          "Por favor, selecione o paciente para iniciar a gravação da consulta.",
          { preferVoice: false },
        );
        return;
      }
      ...
      if (
        ...
        sendMessage("Reconhecimento de voz não suportado neste navegador.", {
          preferVoice: false,
        });
        return;
      }
      ...
      try {
        ...
        sendMessage(
          '🎙️ Gravação de consulta iniciada. Diga "Parar gravação" para finalizar.',
          { preferVoice: false },
        );
      } catch (e) {
        console.error("Erro ao iniciar gravação:", e);
        sendMessage("Erro ao iniciar gravação. Tente novamente.", {
          preferVoice: false,
        });
      }
    }, [
    ...
    const handleStopConsultationRecording = useCallback(async () => {
      ...
      const duration = consultationStartTime
        ? Math.round(
            (endTime.getTime() - consultationStartTime.getTime()) / 1000 / 60,
          ) // minutos
        : 0;
      ...
      try {
        ...
        const { data: assessment, error: assessmentError } = await supabase
          ...
          .insert({
            ...
            created_at:
              consultationStartTime?.toISOString() || new Date().toISOString(),
        ...
        sendMessage(
          `✅ Consulta gravada e salva com sucesso! Duração: ${duration} minutos.`,
          { preferVoice: false },
        );
        ...
        sendMessage(
          `❌ Erro ao salvar consulta: ${error.message || "Erro desconhecido"}`,
          { preferVoice: false },
        );
      } finally {
    ...
    const processFileUpload = useCallback(async () => {
      ...
      try {
        ...
        sendMessage(
          `📤 Enviando documento "${uploadedFile.name}" para a biblioteca e base de conhecimento...`,
          { preferVoice: false },
        );
        ...
        const baseFileName = `${Date.now()}_${uploadedFile.name.replace(/[^a-zA-Z0-9.-]/g, "_")}`;
        ...
        // Use user_id folder (required by RLS ownership policy)
        const fileName = `${currentUser.id}/${baseFileName}`;
        ...
        const calculateAIRelevance = (
          ...
          if (
            ...
            name.includes("diretriz") ||
            name.includes("manual") ||
            text.includes("protocolo clínico") ||
            text.includes("diretriz terapêutica")
          ) {
            return 1.0;
          }
        ...
        const dynamicRelevance = calculateAIRelevance(
          uploadedFile.name,
          extractedContent,
        );
        ...
        if (docError) {
          ...
          if (
            ...
            docError.message?.includes("403")
          ) {
            throw new Error(
              "Erro de permissão (403). As políticas RLS da tabela documents precisam ser configuradas. Execute o script FIX_RLS_DOCUMENTS_TABLE.sql no Supabase SQL Editor.",
            );
          }
        ...
        if (!documentData || !documentData.id) {
          throw new Error(
            "Documento não foi salvo corretamente no banco de dados",
          );
        }
        ...
        if (
          documentData?.id &&
          (uploadCategory === "ai-documents" || uploadCategory === "research")
        ) {
          try {
            await KnowledgeBaseIntegration.linkDocumentToAI(
              documentData.id,
              documentMetadata.aiRelevance || 0.8,
            );
            console.log("✅ Documento vinculado à IA:", documentData.id);
          } catch (linkError) {
            console.warn(
              "⚠️ Erro ao vincular documento à IA (não crítico):",
              linkError,
            );
          }
        }
        ...
        if (verifyDoc.error) {
          console.warn(
            "⚠️ Documento não encontrado imediatamente após inserção:",
            verifyDoc.error,
          );
        } else {
        ...
        sendMessage(
          `❌ Erro ao fazer upload do documento "${uploadedFile?.name}": ${error.message || "Erro desconhecido"}. Por favor, tente novamente.`,
          { preferVoice: false },
        );
    ...
    const extractTextFromPDF = useCallback(
      async (file: File): Promise<string> => {
        try {
          ...
          console.log(
            `✅ Extração concluída: ${fullText.length} caracteres de ${pdf.numPages} páginas`,
          );
          return (
            fullText.trim() ||
            "Documento PDF processado, mas nenhum texto legível foi extraído."
          );
        } catch (error) {
      ...
      <>
                  ...
                  <>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <button
                        onClick={() =>
                          sendMessage(
                            "Analise o documento que acabei de abrir no chat.",
                            {
                              documentForAnalysis: {
                                id: inlineDoc.id,
                                title: inlineDoc.title ?? "",
                    ...
                    <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 max-h-[40vh] overflow-y-auto">
                      ...
                      <pre className="whitespace-pre-wrap break-words text-slate-100 text-sm leading-relaxed">
                        {sanitizeInlineText(inlineDoc.content || "") ||
                          "Sem conteúdo textual disponível para este documento."}
                      </pre>
                    </div>
                  </>
                )}
        ...
        {!hideButton && !isOpen && (
          <button
            ...
            style={{
              background:
                "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f3a3a 100%)",
            }}
        ...
        {isOpen && (
          <div
            ...
            style={{
              ...
              top:
                position !== "inline" && !isExpanded && position.includes("top")
                  ? "1rem"
                  : undefined,
              bottom:
                position !== "inline" &&
          ...
          >
            ...
            {isOffline && (
              <div
                className="border-b border-yellow-900/30 bg-yellow-950/30 px-3 sm:px-5 py-1.5 flex items-center gap-2 text-[10px] sm:text-xs text-yellow-400/90 flex-shrink-0"
              ...
              >
                ...
                />
                <span>
                  💛 Modo local ativo — funções operacionais disponíveis
                </span>
              </div>
            )}
            ...
            >
                ...
                (resolvedVariant === "clean" ? (
                  <div className="h-full flex flex-col items-center justify-center relative min-h-[50vh] animate-in fade-in duration-1000">
                    ...
                    <div className="relative w-40 h-40 mb-8 transform transition-transform duration-[4000ms] hover:scale-105">
                      ...
                      <div
                        className="absolute bottom-4 -right-2 w-1 h-1 bg-cyan-400 rounded-full shadow-[0_0_8px_rgb(34,211,238)] animate-bounce"
                    ...
                    <div className="text-center space-y-3 z-10">
                      ...
                      {/* 🧬 [TITAN 5.2] PROGRESS BAR INTELLIGENCE */}
                      {(() => {
                          const lastMsg = messages[messages.length - 1];
                          const meta = (lastMsg?.metadata as any)?.metadata ?? lastMsg?.metadata;
                          const phase = meta?.assessmentPhase || meta?._aec_layers?.phase;
                          const progress = PHASE_PROGRESS[phase as string] || 0;
                          
                          if (progress <= 0 || progress >= 100) return (
                              <div className="flex gap-2 mt-6 opacity-60 justify-center">
                          ...
                          return (
                              <div className="mt-6 w-full max-w-[200px] mx-auto space-y-2">
                                  ...
                                  <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden border border-slate-700/50">
                                      <motion.div 
                                          ...
                                          className="h-full bg-gradient-to-r from-emerald-500 via-cyan-400 to-emerald-500 shadow-[0_0_10px_rgba(52,211,153,0.4)]"
                                      />
                                  </div>
                              </div>
                          );
              ...
              {messages.map((message) => {
                        ...
                        <div className="p-4 sm:p-5">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-1">
                              <h3 className="font-semibold text-emerald-100 text-base sm:text-lg">
                                Avaliação Concluída
                              </h3>
                              <p className="text-emerald-200/80 text-sm leading-relaxed whitespace-pre-wrap">
                                {stripActionTokenForDisplay(
                ...
                const coreMeta =
                  (message.metadata as any)?.metadata ?? message.metadata;
                const triggerScheduling =
                  (message.metadata as any)?.trigger_scheduling === true ||
                  coreMeta?.trigger_scheduling === true;
                const professionalIdFromMeta =
                  (message.metadata as any)?.professionalId ??
                  coreMeta?.professionalId;
                        ...
                        >
                          ...
                          {(message.metadata as any)?.intent && (
                            <div className="mt-1 pt-1 border-t border-white/10 text-[9px] opacity-50 font-mono hidden group-hover:block">
                              Intent: {(message.metadata as any).intent} | Prof:{" "}
                ...
                const buttonCommands = meta?.buttonCommands as
                  | typeof appCommands
                  | undefined;
                const allNavSources =
                  (buttonCommands?.length ? buttonCommands : appCommands) ?? [];
                const navCommands = allNavSources.filter(
                  (c) =>
                    c?.kind === "noa_command" &&
                    c?.command &&
                    (c.command.type === "navigate-section" ||
                      c.command.type === "navigate-route"),
                );
                ...
                return (
                  ...
                  >
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                window.dispatchEvent(
                                  new CustomEvent("noaCommand", { detail }),
                                );
                              }}
                              className={
                                isPostAec
                    ...
                    {message.role === "noa" &&
                      (() => {
                        const total = (meta?.documents_total ??
                          (meta?.metadata as Record<string, unknown>)
                            ?.documents_total) as number | undefined;
                        if (typeof total !== "number" || total < 0) return null;
                        const block = (meta?.documents_block_size ??
                          (meta?.metadata as Record<string, unknown>)
              ...
              style={{
                ...
                backgroundColor:
                  resolvedVariant === "clean"
                    ...
                    : "rgb(15 23 42 / 0.95)",
                backdropFilter:
                  resolvedVariant === "clean" ? "none" : "blur(8px)",
              }}
            >
              {error && (
                <div className="text-xs text-amber-400 px-1">{error}</div>
              )}
                ...
                >
                  <button
                    ...
                    title={
                      isUploading
                        ? "Enviando documento..."
                        : "Enviar documento para biblioteca"
                    }
                  ...
                  onChange={(event) => {
                    if (!isProcessing) {
                      ...
                      if (
                        ...
                        event.target.value !== recognitionRef.current.buffer
                      ) {
                ...
                {isProcessing && (
                  <button
                    onClick={(e) => {
                      ...
                      e.stopPropagation();
                      console.log(
                        "🔄 Resetando estado de processamento manualmente",
                      );
                      // Forçar reset do estado imediatamente
                      if (
                        window.confirm(
                          "A interface está travada. Deseja recarregar a página?",
                        )
                      ) {
                    ...
                    {[
                      {
                        id: "ai-documents",
                        name: "🧠 IA Residente",
                        desc: "Treinar a Nôa Esperança",
                      },
                      {
                        id: "protocols",
                        name: "📖 Protocolos",
                        desc: "Diretrizes clínicas",
                      },
                      {
                        id: "research",
                        name: "🔬 Pesquisa",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      const isSelected =
                        uploadUserType.includes(type.id) ||
                        (type.id === "all" && uploadUserType.length === 3);
                      return (
                        <button
                          key={type.id}
                          onClick={() => {
