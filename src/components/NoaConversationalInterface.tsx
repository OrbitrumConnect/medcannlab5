    ...
    <div className="bg-slate-800/80 rounded-lg p-4 mb-4 border border-slate-700 w-full animate-in fade-in zoom-in duration-300">
      <div className="flex items-center justify-between mb-3 border-b border-slate-700 pb-2">
        <div>
          ...
          {professionalName && (
            <p className="text-xs text-emerald-400 mt-0.5 ml-6">com {professionalName}</p>
          )}
        </div>
...
const NoaConversationalInterface = React.forwardRef<HTMLDivElement, NoaConversationalInterfaceProps>(({
  userCode = 'DR-001',
    ...
    <>
      ...
      {!hideButton && !isOpen && (
        <button
          ...
          className={clsx('fixed z-50 w-16 h-16 rounded-full shadow-xl hidden md:flex items-center justify-center text-white overflow-hidden border border-[#00C16A]/30 transition-transform duration-300 hover:scale-110', positionClasses)}
                  ...
                  {message.role === 'noa' && dedupedNav.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1.5 max-w-[85%] sm:max-w-[80%]">
                      {dedupedNav.map((entry, idx) => {
                        ...
                        const isPostAec = label.includes('📋') || label.includes('📅')
                        return (
                          <button
                            key={idx}
                            type="button"
