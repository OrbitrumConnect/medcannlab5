...
import { Mic, MicOff, X, Send, Loader2, Activity, BookOpen, Brain, Upload, Maximize2, Minimize2, User, MessageSquare, Clock } from 'lucide-react'
...
const NoaConversationalInterface: React.FC<NoaConversationalInterfaceProps> = ({
  ...
  // Verificar se há chat aberto no iframe (para evitar duplicação)
  const [isChatActiveInIframe, setIsChatActiveInIframe] = useState(() => {
    if (typeof document !== 'undefined') {
      return document.body.hasAttribute('data-chat-active')
    }
    return false
  })
  ...
  useEffect(() => {
    const checkChatActive = () => {
      setIsChatActiveInIframe(document.body.hasAttribute('data-chat-active'))
    }
    ...
    if (document.body) {
      observer.observe(document.body, {
        attributes: true,
        attributeFilter: ['data-chat-active']
      })
    }
  ...
  // Se o chat está aberto no iframe e este NOA não está em modo embed, não abrir automaticamente
  const shouldAutoOpen = hideButton || (contextIsOpen && !isChatActiveInIframe)
  ...
  useEffect(() => {
    // Se o chat está aberto no iframe, não abrir o chat do Layout
    if (isChatActiveInIframe && !isEmbedMode) {
      setIsOpen(false)
    } else {
      setIsOpen(contextIsOpen || hideButton)
    }
  }, [contextIsOpen, hideButton, isChatActiveInIframe, isEmbedMode])
  ...
  // Buscar histórico de conversas quando em modo embed
  useEffect(() => {
    if (isEmbedMode && isOpen && user?.id) {
      const loadConversationHistory = async () => {
        setLoadingHistory(true)
        try {
          // Buscar conversas anteriores do usuário com a Nôa
          // Assumindo que há uma tabela ou forma de identificar conversas com a Nôa
          const { data, error } = await supabase
            .from('chat_messages')
            .select('*, conversation_id, created_at, content, role')
            .eq('user_id', user.id)
            .or('role.eq.noa,role.eq.assistant')
            .order('created_at', { ascending: false })
            .limit(50)

          if (error) {
            console.warn('Erro ao buscar histórico:', error)
            // Tentar buscar de outra forma - agrupar por conversation_id
            ...
            if (groupedData) {
              // Agrupar por conversation_id e pegar a primeira mensagem de cada conversa
              const conversationsMap = new Map()
              groupedData.forEach((msg: any) => {
                const convId = msg.conversation_id || 'default'
                if (!conversationsMap.has(convId)) {
                  conversationsMap.set(convId, {
                    id: convId,
                    preview: msg.content?.substring(0, 100) || 'Conversa',
                    date: msg.created_at,
                    messageCount: 1
                  })
                } else {
              ...
              setConversationHistory(Array.from(conversationsMap.values()))
            }
          } else if (data) {
  ...
  return (
    <>
      {!hideButton && !isOpen && (
        <button
          onClick={() => {
            // Se o chat está aberto no iframe, não abrir o chat do Layout para evitar duplicação
            if (!isChatActiveInIframe || isEmbedMode) {
              setIsOpen(true)
            }
          }}
      ...
      {isOpen && (
        <div className={clsx(
          'fixed z-50 bg-slate-900/95 border border-slate-700 rounded-3xl shadow-2xl backdrop-blur-xl transition-all duration-300',
          isEmbedMode
            ? 'left-4 top-4 bottom-4 flex gap-4' // Modo embed: container flex para chat + histórico
            : isExpanded 
              ? 'left-[80px] lg:left-[320px] right-4 top-4 bottom-4 flex flex-col' // Expandido: da barra lateral (80px quando colapsada, 320px quando expandida) até a borda direita
              : position === 'bottom-right' 
                ? 'bottom-4 right-4 w-[600px] max-w-[calc(100vw-2rem)] h-[720px] max-h-[calc(100vh-2rem)] flex flex-col'
                : position === 'bottom-left'
                  ? 'bottom-4 left-4 w-[600px] max-w-[calc(100vw-2rem)] h-[720px] max-h-[calc(100vh-2rem)] flex flex-col'
                  : position === 'top-right'
                    ? 'top-4 right-4 w-[600px] max-w-[calc(100vw-2rem)] h-[720px] max-h-[calc(100vh-2rem)] flex flex-col'
                    : 'top-4 left-4 w-[600px] max-w-[calc(100vw-2rem)] h-[720px] max-h-[calc(100vh-2rem)] flex flex-col'
        )}>
          {/* Container principal do chat */}
          <div className={clsx(
            'bg-slate-900/95 border border-slate-700 rounded-3xl shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden',
            isEmbedMode ? 'w-[950px]' : 'flex-1'
          )}>
          ...
          {/* Painel de Histórico - Apenas em modo embed */}
          {isEmbedMode && (
            <div className="w-[320px] bg-slate-900/95 border border-slate-700 rounded-3xl shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden">
              {/* Header do Histórico */}
              <div className="bg-gradient-to-r from-slate-700 to-slate-800 px-4 py-3 border-b border-slate-700">
                <div className="flex items-center gap-2">
                  <MessageSquare className="w-4 h-4 text-slate-300" />
                  <h3 className="text-sm font-semibold text-white">Histórico de Conversas</h3>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-2">
                {loadingHistory ? (
                  ...
                  <div className="text-center py-8 px-4">
                    ...
                    <p className="text-xs text-slate-400">Nenhuma conversa anterior</p>
                  </div>
                ) : (
                  conversationHistory.map((conv) => (
                    <button
                      key={conv.id}
                      onClick={() => setSelectedHistoryItem(conv.id === selectedHistoryItem ? null : conv.id)}
                      className={clsx(
                        'w-full text-left p-3 rounded-lg border transition-all',
                        selectedHistoryItem === conv.id
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-[10px] text-slate-500">
                          {conv.date ? new Date(conv.date).toLocaleDateString('pt-BR', { 
                            day: '2-digit', 
                            month: 'short',
                            year: new Date(conv.date).getFullYear() !== new Date().getFullYear() ? 'numeric' : undefined
                          }) : 'Data não disponível'}
                        </span>
                        {conv.messageCount > 0 && (
