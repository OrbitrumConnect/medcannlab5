        ...
        <div className="space-y-4">
              ...
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-brand-surface-subtle text-brand-text px-10 py-2 rounded-md border border-slate-600 focus:border-purple-500 focus:outline-none"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPatients.map((patient) => (
              <div key={patient.id} className="bg-brand-surface border border-brand-border rounded-lg p-6 hover:border-slate-600 transition-colors">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-brand-text-muted text-sm">{patient.age} anos</p>
                    </div>
                  </div>
                ...
                <div className="mb-4">
                  <p className="text-brand-text-muted text-sm mb-2">Medicações:</p>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-brand-surface border border-brand-border rounded-lg p-4">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <select
                ...
                className="bg-brand-surface-subtle text-brand-text px-3 py-2 rounded-md border border-slate-600 focus:border-purple-500 focus:outline-none"
              >
          ...
          <div className="space-y-3">
            {filteredEvents.map((event) => (
              <div key={event.id} className="bg-brand-surface border border-brand-border rounded-lg p-4 hover:border-slate-600 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="text-brand-text font-semibold">{event.patientName}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                  <div>
                    ...
                    <p className="text-brand-text font-semibold">{event.duration}s</p>
                  </div>
                  <div>
                    <p className="text-brand-text-muted text-sm">Gatilhos</p>
                    <div className="flex flex-wrap gap-1">
                  ...
                  <div>
                    <p className="text-brand-text-muted text-sm">Medicações</p>
                    <div className="flex flex-wrap gap-1">
                ...
                {event.wearableData && (
                  <div className="bg-brand-surface-subtle rounded-lg p-3 mb-3">
                    <p className="text-brand-text-muted text-sm mb-2">Dados do Wearable:</p>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <div className="text-center">
                        <Heart className="w-4 h-4 text-red-400 mx-auto mb-1" />
                        <p className="text-brand-text font-semibold">{event.wearableData.heartRate} bpm</p>
                        <p className="text-brand-text-muted text-xs">Frequência Cardíaca</p>
                      </div>
                      <div className="text-center">
                        <Activity className="w-4 h-4 text-blue-400 mx-auto mb-1" />
                        <p className="text-brand-text font-semibold">{event.wearableData.oxygenSaturation}%</p>
                        <p className="text-brand-text-muted text-xs">Saturação O2</p>
                      </div>
                      <div className="text-center">
                        <TrendingUp className="w-4 h-4 text-green-400 mx-auto mb-1" />
                        <p className="text-brand-text font-semibold">{event.wearableData.movement}</p>
                        <p className="text-brand-text-muted text-xs">Movimento</p>
                      </div>
                      <div className="text-center">
                        <Clock className="w-4 h-4 text-yellow-400 mx-auto mb-1" />
                        <p className="text-brand-text font-semibold">{event.wearableData.temperature}°C</p>
                ...
                {event.notes && (
                  <p className="text-brand-text-secondary text-sm">{event.notes}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Dispositivos Conectados</h3>
                <Activity className="w-5 h-5 text-green-400" />
              </div>
              <p className="text-3xl font-bold text-brand-text">
                {patients.filter(p => p.wearableConnected).length}
              </p>
              <p className="text-brand-text-muted text-sm">de {patients.length} pacientes</p>
            </div>
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Monitoramento Ativo</h3>
                <Eye className="w-5 h-5 text-blue-400" />
              </div>
              <p className="text-3xl font-bold text-brand-text">24/7</p>
              <p className="text-brand-text-muted text-sm">Tempo real</p>
            </div>
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Alertas Hoje</h3>
                <AlertTriangle className="w-5 h-5 text-yellow-400" />
              </div>
              <p className="text-3xl font-bold text-brand-text">3</p>
              <p className="text-brand-text-muted text-sm">Eventos detectados</p>
            </div>
          </div>

          {/* Lista de Dispositivos */}
          <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
            <h3 className="text-brand-text font-semibold mb-4">Dispositivos Wearables</h3>
            <div className="space-y-3">
              {patients.filter(p => p.wearableConnected).map((patient) => (
                <div key={patient.id} className="flex items-center justify-between p-3 bg-brand-surface-subtle rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-brand-text-muted text-sm">Conectado</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Total de Pacientes</h3>
            ...
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Eventos Este Mês</h3>
                <Zap className="w-5 h-5 text-yellow-400" />
              </div>
              <p className="text-3xl font-bold text-brand-text">{events.length}</p>
              <p className="text-red-400 text-sm">-15% vs mês anterior</p>
            </div>
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Respondedores TEZ</h3>
            ...
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-brand-text font-semibold">Monitoramento</h3>
                <Activity className="w-5 h-5 text-blue-400" />
              </div>
              <p className="text-3xl font-bold text-brand-text">
                {Math.round((patients.filter(p => p.wearableConnected).length / patients.length) * 100)}%
              </p>
              <p className="text-brand-text-muted text-sm">Cobertura</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <h3 className="text-brand-text font-semibold mb-4">Tipos de Epilepsia</h3>
              <div className="space-y-3">
            ...
            <div className="bg-brand-surface border border-brand-border rounded-lg p-6">
              <h3 className="text-brand-text font-semibold mb-4">Status TEZ</h3>
              <div className="space-y-3">
                {['candidato', 'em_tratamento', 'respondedor', 'nao_respondedor'].map((status) => (
                  <div key={status} className="flex items-center justify-between">
                    <span className="text-brand-text-secondary">{getTEZStatusText(status)}</span>
                    <div className="flex items-center space-x-2">
                      <div className="w-20 bg-brand-surface-subtle rounded-full h-2">
                        <div 
