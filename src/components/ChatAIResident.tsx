...
const ChatAIResident: React.FC<ChatAIResidentProps> = ({
  ...
  const saveTranscriptToHistory = async (transcript: string) => {
    try {
      ...
      // Salvar transcrição no histórico clínico via clinical_assessments
      ...
      if (historyError) {
        console.error('Erro ao salvar transcrição:', historyError)
      }
    } catch (error) {
  ...
  const saveAudioToHistory = async (audioBlob: Blob, transcript: string) => {
    try {
      ...
      try {
        ...
        const fileName = `${currentUser.id}/chat-audio-${chatId}-${Date.now()}.webm`
      ...
      // Salvar no histórico clínico via clinical_assessments
      const { error: historyError } = await supabase
        .from('clinical_assessments')
        .insert({
          patient_id: patientId,
      ...
      if (historyError) {
        console.error('Erro ao salvar no histórico clínico:', historyError)
      }
    } catch (error) {
