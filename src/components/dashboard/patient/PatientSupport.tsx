...
export const PatientSupport: React.FC<PatientSupportProps> = ({ onSuccess }) => {
    ...
    const handleSend = async () => {
        ...
        setSending(true)
        try {
            const { data: admins } = await supabase.from('users_compatible').select('id').eq('type', 'admin').limit(10)
            ...
            if (existing?.length) {
                ...
                if (newRoom) {
                    roomId = newRoom.id
                    const participants = [{ room_id: roomId, user_id: user.id, role: 'patient' }, ...admins.map(a => ({ room_id: roomId, user_id: a.id, role: 'admin' }))]
