    ...
    return (
        <div className="space-y-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="space-y-2">
                ...
                <p className="text-brand-text-secondary text-sm md:text-base">Gerencie suas consultas e visualize seu calendário</p>
            </div>

            <div className="flex gap-2">
                <button onClick={() => setViewMode('calendar')} className={`px-4 py-2 rounded-lg font-semibold transition-all ${viewMode === 'calendar' ? 'bg-primary-500 text-brand-text shadow-lg shadow-primary-500/20' : 'bg-slate-800/50 text-brand-text-secondary hover:bg-brand-surface-subtle'}`}>Calendário</button>
                <button onClick={() => setViewMode('list')} className={`px-4 py-2 rounded-lg font-semibold transition-all ${viewMode === 'list' ? 'bg-primary-500 text-brand-text shadow-lg shadow-primary-500/20' : 'bg-slate-800/50 text-brand-text-secondary hover:bg-brand-surface-subtle'}`}>Lista</button>
            </div>
            ...
            <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 backdrop-blur-xl">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-semibold text-brand-text">Avaliação Clínica</h2>
                            <p className="text-brand-text-muted text-sm">Realize sua avaliação completa com a IA Nôa</p>
                        </div>
                    </div>
                    <button onClick={onStartAssessment} className="px-6 py-2.5 bg-primary-500 hover:bg-primary-400 text-brand-text rounded-xl font-semibold transition-all flex items-center gap-2 shadow-lg shadow-primary-500/25">
            ...
            <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 backdrop-blur-xl space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={onScheduleNew} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-brand-text rounded-xl text-sm font-semibold transition-all border border-white/10 flex items-center gap-2">
                        ...
                        <div key={apt.id} className="group bg-white/5 hover:bg-white/10 rounded-2xl p-4 border border-white/5 transition-all flex items-center justify-between backdrop-blur-md">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-brand-text-muted text-sm">{apt.date ? new Date(apt.date).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' }) : '—'} às {apt.time}</p>
                                </div>
                            </div>
            ...
            {viewMode === 'calendar' && (
                <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-8 backdrop-blur-2xl">
                    <div className="flex justify-between items-center mb-8">
                        <h3 className="text-2xl font-bold text-brand-text">{monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}</h3>
                        <div className="flex gap-2">
                        ...
                        {calendarDays.map((day, i) => (
                            <button
                                ...
                                className={`aspect-square rounded-2xl flex flex-col items-center justify-center gap-1 transition-all border ${!day.isCurrentMonth ? 'opacity-20 pointer-events-none' :
                                        day.isToday ? 'bg-primary-500/20 border-primary-500/50 text-brand-text' :
                                            selectedDate?.toDateString() === day.fullDate.toDateString() ? 'bg-primary-500 border-primary-500 text-brand-text shadow-lg shadow-primary-500/40' :
