  ...
  const loadSharedReports = async () => {
    try {
      ...
      const formattedReports: SharedReport[] = sharedReports?.map(report => {
        const content = report.content as Record<string, any> | null ?? {}
        return {
          id: report.id,
          patientId: report.patient_id,
          patientName: report.patient_name || 'Paciente',
          ...
          assessmentType: (content.assessment_type as string) || 'initial_assessment',
          ...
          content: {
            chiefComplaint: (content.chief_complaint as string) || '',
            history: (content.history as string) || '',
            physicalExam: (content.physical_exam as string) || '',
