...
const AssessmentRequiredModal: React.FC<AssessmentRequiredModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-in fade-in duration-200 backdrop-blur-sm">
            <div className="bg-brand-bg border border-brand-border rounded-2xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 space-y-4">
                    ...
                    <p className="text-center text-brand-text-secondary">
                        Para agendar com <strong className="text-brand-text">{professionalName || 'nossos especialistas'}</strong>,
                        é necessário realizar primeiramente sua <strong className="text-emerald-400">Avaliação Clínica Inicial</strong>.
                    </p>

                    <div className="bg-slate-800/50 rounded-xl p-4 border border-brand-border text-sm space-y-2">
                        <div className="flex gap-2">
                            <div className="min-w-[4px] bg-emerald-500 rounded-full"></div>
                            <p className="text-brand-text-muted">A avaliação é conduzida pela Nôa (IA) e é totalmente sigilosa.</p>
                        </div>
                        <div className="flex gap-2">
                            <div className="min-w-[4px] bg-blue-500 rounded-full"></div>
                            <p className="text-brand-text-muted">Seu relatório será salvo e enviado automaticamente para o profissional escolhido.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-5 border-t border-slate-700/50 flex flex-col gap-3">
                    <button
                        ...
                        className="w-full bg-blue-600 hover:bg-blue-500 text-brand-text py-3 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all hover:scale-[1.02] shadow-lg shadow-blue-500/20"
                    >
                    ...
                    <button
                        onClick={onClose}
                        className="w-full bg-brand-surface hover:bg-brand-surface-subtle text-brand-text-secondary py-3 rounded-xl font-medium transition-colors"
                    >
                        Voltar
                    </button>
                </div>
