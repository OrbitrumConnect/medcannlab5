  ...
  useEffect(() => {
    const handleNoaCommand = (event: Event) => {
      ...
      if (detail.type === 'navigate-route' && typeof detail.target === 'string' && detail.target.length > 0) {
        let safeTarget = detail.target.trim()
        
        // 🛡️ [SECURITY: OPEN REDIRECT / XSS]
        if (
          safeTarget.startsWith('http://') || 
          safeTarget.startsWith('https://') || 
          safeTarget.toLowerCase().startsWith('javascript:') ||
          safeTarget.toLowerCase().startsWith('data:')
        ) {
          console.warn('⚠️ [SECURITY] Navegação insegura bloqueada via navigate-route:', safeTarget)
          safeTarget = '/app'
        }
      ...
      if (detail.type === 'navigate-section') {
        let fallback = typeof detail.fallbackRoute === 'string' ? detail.fallbackRoute.trim() : ''
        const section = typeof detail.target === 'string' ? detail.target.trim() : ''
        ...
        if (
          ...
          fallback.toLowerCase().startsWith('javascript:') ||
          fallback.toLowerCase().startsWith('data:')
        ) {
          console.warn('⚠️ [SECURITY] Fallback inseguro bloqueado via navigate-section:', fallback)
          fallback = '/app'
        }
