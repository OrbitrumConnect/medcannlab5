...
import ProfessionalSchedulingWidget from './ProfessionalSchedulingWidget'
...
const IntegratedWorkstation: React.FC<IntegratedWorkstationProps> = ({ initialTab, defaultPatientId }) => {
    ...
    const allTabs: { id: TabId; label: string; icon: typeof Users; color: string; group: TabGroup }[] = [
        ...
        { id: 'team', label: 'Equipe Clínica', icon: UsersRound, color: 'text-indigo-400', group: 'atendimento' },
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'governance':
                return (
                    <div className="h-full overflow-y-auto bg-[#0f172a]">
                        <div className="px-4 py-2 border-b border-slate-700/50 bg-slate-800/30 flex items-center gap-2 text-brand-text-muted text-sm">
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] bg-[#0f172a] w-full max-w-full overflow-hidden" data-integrated-terminal>
            ...
            <header className="bg-gradient-to-r from-slate-900 via-slate-800 to-emerald-900 border-b border-[#334155] shrink-0 z-10 shadow-sm relative w-full min-h-[48px] flex flex-col">
                ...
                <div className="hidden sm:flex items-center px-3 w-full h-12">
                    <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="leading-tight">
                            <h1 className="text-sm font-bold text-brand-text leading-none">Terminal Integrado</h1>
                            <p className="text-[9px] uppercase tracking-wider text-brand-text-muted font-semibold mt-0.5">MedCannLab OS</p>
                        </div>
                    </div>
                ...
                {/* Mobile: grid compacto de tabs (2 linhas de 5) */}
