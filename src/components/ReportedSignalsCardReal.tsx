...
export default function ReportedSignalsCardReal() {
  ...
  if (cases.length === 0) {
    return (
      <div className="rounded-xl border border-teal-500/30 bg-gradient-to-br from-teal-900/20 via-slate-900/60 to-cyan-900/20 p-4 flex flex-col items-center justify-center min-h-[200px] text-center">
        <Ear className="w-6 h-6 text-teal-300/40 mb-2" />
        <span className="text-[11px] font-semibold text-teal-200">Sinais do Relato</span>
  ...
  return (
    <div className="rounded-xl border border-teal-500/30 bg-gradient-to-br from-teal-900/20 via-slate-900/60 to-cyan-900/20 backdrop-blur-sm overflow-hidden flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-teal-500/20 bg-teal-500/5">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-6 h-6 rounded-lg bg-teal-500/20 border border-teal-500/30 flex items-center justify-center flex-shrink-0">
            <Ear className="w-3.5 h-3.5 text-teal-300" />
          </div>
          <div className="min-w-0">
            <h3 className="text-sm font-semibold text-teal-100 flex items-center gap-1.5 truncate">
              Sinais do Relato
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-teal-500/20 text-teal-200 font-normal flex-shrink-0">
                DOR · SONO · ANSIEDADE
              </span>
            </h3>
      ...
      <div className="px-3 py-3 space-y-2.5 flex-1 flex flex-col">
        <div className="flex items-center justify-between text-[10px] text-slate-400 pb-1.5 border-b border-teal-500/10">
          <button
            type="button"
            onClick={() => navigate(`/app/patients?patientId=${current.patientId}&tab=charts`)}
            className="font-medium text-teal-200/90 truncate hover:text-teal-100 hover:underline text-left"
            ...
            <div key={s.id} className="flex items-start gap-1.5 text-[10px] leading-tight">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-teal-200/90 font-medium truncate">{s.subcategoria}</span>
                </div>
        ...
        <button
          type="button"
          onClick={() => navigate(`/app/patients?patientId=${current.patientId}&tab=charts`)}
          className="w-full px-2 py-1.5 bg-teal-500/15 hover:bg-teal-500/30 border border-teal-500/30 text-teal-200 rounded-md text-[10px] font-semibold flex items-center justify-center gap-1 transition-colors"
        ...
        <div className="mt-auto pt-1.5 border-t border-teal-500/10">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-3 mt-2 pt-2 border-t border-teal-500/10">
              <button
                ...
                disabled={page === 1}
                className="p-1.5 rounded-lg bg-teal-500/15 hover:bg-teal-500/30 border border-teal-500/30 text-teal-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                aria-label="Caso anterior"
              >
                <ChevronLeft className="w-4 h-4" />
              </button>
              <span className="text-[10px] text-teal-200/80 font-medium min-w-[64px] text-center">
