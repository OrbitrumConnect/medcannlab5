  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="flex items-center space-x-3 p-3 rounded-lg bg-brand-surface-subtle hover:bg-slate-600 transition-colors"
      >
        ...
        <div className="text-left">
          <div className="text-brand-text font-medium">{currentType.name}</div>
          <div className="text-sm text-brand-text-muted">{currentType.description}</div>
        </div>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-80 bg-brand-surface rounded-xl shadow-xl border border-brand-border z-50">
          <div className="p-4">
            <h3 className="text-lg font-semibold text-brand-text mb-4">Selecionar Área de Atuação</h3>
              ...
              {userTypes.map((type) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-left">
                      <div className="text-brand-text font-medium">{type.name}</div>
                      <div className="text-sm text-brand-text-muted">{type.description}</div>
                    </div>
                  </button>
                )
              })}
                  ...
                  {serviceAreas.map((area, index) => {
                    ...
                    return (
                      <a
                        key={index}
                        href={area.href}
                        className="flex items-center space-x-3 p-2 rounded-lg hover:bg-brand-surface-subtle transition-colors"
                      >
                        <Icon className="w-4 h-4 text-brand-text-muted" />
                        <span className="text-sm text-brand-text-secondary">{area.name}</span>
                      </a>
                    )
                  })}
                </div>
            ...
            <div className="mt-6 pt-4 border-t border-brand-border">
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <span className="text-sm text-brand-text-secondary">Arte da Entrevista Clínica</span>
                </a>
                <a
                  href="/app/cannabis-medicinal-course"
                  className="flex items-center space-x-3 p-2 rounded-lg hover:bg-brand-surface-subtle transition-colors"
                >
                  <Brain className="w-4 h-4 text-brand-text-muted" />
                  <span className="text-sm text-brand-text-secondary">Pós-Graduação Cannabis Medicinal</span>
                </a>
              </div>
            </div>
          </div>
