...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
                  ...
                  return (
                    <div key={axis.key} className="space-y-1">
                      ...
                      {(expandedAxis === axis.key) && (
                        <div
                          className={`space-y-1 ${
                            isCollapsed ? 'ml-0 border-l pl-2' : 'ml-9'
                          } transition-all`}
      ...
      {/* System Stats - Estilo do PatientSidebar */}
      {!isCollapsed && normalizedType === 'admin' && (
        <div className="p-2 border-t" style={{ borderColor: 'rgba(0,193,106,0.18)' }}>
          <h3 className="text-[10px] font-semibold text-[rgba(200,214,229,0.75)] mb-1.5 uppercase tracking-wider">Status do Sistema</h3>
          <div className="space-y-1">
            {systemStats.map((stat, index) => (
              <div key={index} className="flex justify-between items-center text-[11px]">
                <span className="text-[rgba(200,214,229,0.75)] truncate pr-2">{stat.label}</span>
                <span className={`font-medium ${stat.color} flex-shrink-0 text-[11px]`}>{stat.value}</span>
              </div>
            ))}
          </div>
        </div>
