  ...
  const loadPatients = async () => {
    try {
      ...
      assessments?.forEach(assessment => {
        ...
        if (!patientId) return
        const data = assessment.data as Record<string, any> | null
        if (!patientsMap.has(patientId)) {
          patientsMap.set(patientId, {
            id: patientId,
            name: data?.patient_name || `Paciente ${patientId.slice(0, 8)}`,
            age: data?.patient_age || 0,
            cpf: data?.patient_cpf || '',
            phone: data?.patient_phone || '',
            lastVisit: new Date(assessment.created_at ?? Date.now()).toLocaleDateString('pt-BR'),
            status: assessment.status || 'Aguardando',
            condition: data?.condition || 'Epilepsia',
