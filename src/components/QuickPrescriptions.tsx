...
const QuickPrescriptions: React.FC<QuickPrescriptionsProps> = ({ className = '', patientId }) => {
  ...
  // [V1.9.134/135 + V1.9.180-B] Stats + lista cards + view/edit interativos
  ...
  // V1.9.265 — seletor de paciente inline (em vez de mandar pra outra aba)
  const [examPatientLocal, setExamPatientLocal] = useState<{ id: string; name: string } | null>(null)
  ...
  const loadMyPrescriptions = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('id, status, prescription_type, patient_id, patient_name, patient_cpf, patient_email, patient_phone, professional_name, professional_crm, professional_specialty, medications, notes, created_at, signature_timestamp, iti_validation_code, iti_validation_url, iti_qr_code, expires_at, document_level')
  ...
  return (
    <div className={`space-y-6 px-4 md:px-6 lg:px-8 py-4 md:py-6 ${className}`}>
              ...
              {filteredMyPrescriptions.slice(0, 12).map((p) => {
                ...
                return (
                  <button
                    ...
                    title={p.status === 'draft' ? 'Clique para editar este rascunho' : 'Clique para ver detalhes'}
                    className="group relative rounded-xl border border-white/5 bg-gradient-to-br from-slate-800/40 to-slate-900/20 p-4 hover:border-emerald-500/40 hover:from-slate-800/60 hover:shadow-lg hover:shadow-emerald-500/5 transition-all overflow-hidden text-left w-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
                  >
                    {/* Glow effect at top */}
      ...
      {/* V1.9.264/265 — Modal Solicitar Exame (Ricardo 13/05): reuso ExamRequestModule + seletor inline.
          Resolucao paciente: prop patientId (contexto Terminal) -> examPatientLocal (escolhido aqui dentro) -> seletor. */}
      {showExamModal && (() => {
        const effectivePatientId = patientId || examPatientLocal?.id || ''
        const effectivePatientName = patientId
          ? patientsList.find(p => p.id === patientId)?.name
          : examPatientLocal?.name
        const hasPatient = !!effectivePatientId
        const filteredPatients = examPatientSearch.trim()
          ? patientsList.filter(p => p.name?.toLowerCase().includes(examPatientSearch.toLowerCase()))
          : patientsList
        const closeExamModal = () => {
          setShowExamModal(false)
          setExamPatientLocal(null)
          setExamPatientSearch('')
        }
        return (
