...
const DEFAULT_AVATAR_URL = '/AvatarsEstatico.png'
...
const NoaAnimatedAvatar: React.FC<NoaAnimatedAvatarProps> = ({
  ...
  const [avatarUrl, setAvatarUrl] = useState<string>('/AvatarsEstatico.png')
    ...
    const fetchAvatarUrl = async () => {
      try {
        ...
        if (data && data.length > 0 && data[0].id) {
          ...
          console.log('ℹ️ Nenhum avatar válido encontrado no storage, usando fallback local.')
            ...
            <div className="relative w-full h-full flex items-center justify-center bg-gradient-to-br from-[#1e3a8a]/20 to-[#92400e]/20">
              <img
                ...
                onError={(e) => {
                  ...
                  // Fallback para imagem estática local
                  const target = e.target as HTMLImageElement
                  // Evitar loop infinito se a imagem local também falhar
                  if (target.src.includes('AvatarsEstatico.png')) {
                    console.error('❌ Imagem local também falhou.')
                    return
                  }
                  target.src = '/AvatarsEstatico.png'
                }}
                onLoad={() => {
