...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Evoluções escritas por você (V1.9.489)', text: 'Evoluções que VOCÊ registrou via Terminal "Nova Evolução" (FOLLOW_UP) aparecem como cards separados dos relatórios AEC (IA Residente). Fonte distinta na seção "Contexto Paciente" — preserva separação semântica entre o que IA produziu e o que VOCÊ escreveu.' },
      { label: 'Toggles fonte paciente (V1.9.488)', text: 'No topo da view, com paciente em foco, 4 pills permitem ligar/desligar cada fonte longitudinal: AEC · Evoluções · Racionalidades · Dossiês prévios. Defaults ON. Desligar fonte = cards somem do grid + seleções daquela fonte saem do chat. Pill cinza disabled quando a fonte não tem dados pra aquele paciente.' },
      { label: 'Organização visual do "Material disponível"', text: 'Cards são agrupados em 3 categorias: 👤 Contexto Paciente (relatórios AEC + evoluções do médico + racionalidades + dossiês prévios) · 🔬 Pesquisa (PubMed + Base de Conhecimento + Bulas) · 📁 Memória / Casos (casos similares + notas + buscas favoritadas). Categoria sem cards é omitida visualmente.' },
