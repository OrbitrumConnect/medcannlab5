...
const Library: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'renal-health', name: 'Saúde Renal', icon: '❤️', count: 0 }
  ]
  ...
  const knowledgeAreas = [
    ...
    { id: 'renal-health', name: 'Saúde Renal', icon: '❤️', color: 'red' }
  ]
...
<html lang="pt-BR">
  ...
  const handleUploadFile = async (file: File, category: string = 'ai-documents') => {
    ...
    try {
      ...
      const fileExt = file.name.split('.').pop()?.toLowerCase()
      const baseFileName = `${Date.now()}_${file.name.replace(/[^a-zA-Z0-9.-]/g, '_')}`
      ...
      // Use user_id folder for documents bucket (required by RLS ownership policy)
      const fileName = bucketName === 'documents' ? `${currentUser.id}/${baseFileName}` : baseFileName
      ...
      // ==========================================
      // ARQUITETURA MADURA: SALVAR METADATA PRIMEIRO
      // ==========================================
      const initialSummary = `Documento enviado em ${new Date().toLocaleDateString('pt-BR')} - Categoria: ${documentCategory}`
      
      const documentMetadata = {
        title: file.name,
        content: '[Processando extração de texto...]',
      ...
      // 1. Inserção Segura no Banco ANTES da Extração (Se falhar a extração, o arquivo já existe)
      ...
      if (docError) {
        console.error('❌ Erro ao salvar metadata primária:', docError)
        throw docError
      }
      
      const insertedDocId = documentData?.[0]?.id;
      console.log('✅ Metadata física estrutural salva! ID:', insertedDocId)

      // ==========================================
      // FASE 2: EXTRAÇÃO DE CONTEÚDO (Isolada, não quebra o documento se falhar)
      // ==========================================
      let extractedContent = ''
      try {
        console.log('📄 Iniciando extração de conteúdo desconectada na Biblioteca...')
        if (fileExt === 'pdf') {
        ...
        if (extractedContent) {
          ...
          // Limitar tamanho para evitar transbordo (máximo 500k caracteres)
          if (extractedContent.length > 500000) {
            extractedContent = extractedContent.substring(0, 500000) + '\n\n[... conteúdo truncado para otimização do AI ...]'
          }
        }
      } catch (extError) {
        console.warn('⚠️ Erro na extração local de conteúdo na Biblioteca:', extError)
        extractedContent = 'Aviso: Documento salvo fisicamente com sucesso. No entanto, o sistema não pôde extrair o texto para leitura de IA automaticamente devido à formatação (Imagens, scans sem OCR, ou arquivos criptografados).'
      }

      // Se conseguiu extrair, nós atualizamos o documento inicial com o Texto e com um Resumo Inteligente
      if (extractedContent && insertedDocId) {
        let dynamicSummary = initialSummary;
        const preview = extractedContent.substring(0, 300).replace(/\n+/g, ' ').trim()
        if (preview.length > 10) {
           dynamicSummary = `${initialSummary}\n\nResumo prévio:\n${preview}${extractedContent.length > 300 ? '...' : ''}`
        }

        console.log('🔄 Atualizando documento com texto extraído...')
        const { error: updateError } = await supabase
          .from('documents')
          .update({ content: extractedContent, summary: dynamicSummary })
          .eq('id', insertedDocId)

        if (updateError) {
           console.warn('⚠️ Falha leve ao injetar texto no documento (Documento existe):', updateError)
        } else {
           console.log('✅ Documento enriquecido com sucesso na base de conhecimento.')
        }
      }
