...
const PatientDashboard: React.FC = () => {
  ...
  // Expor activeTab para o Layout ocultar NOA quando chat estiver aberto
  useEffect(() => {
    if (activeTab === 'chat' || activeTab === 'chat-noa') {
    ...
    try {
      ...
      if (existingRooms && existingRooms.length > 0) {
        ...
        if (roomError || !newRoom) {
          throw roomError ?? new Error('Não foi possível criar a sala de suporte')
        }
  ...
  const renderChatNoa = () => {
    const chatUrl = `/app/chat-noa-esperanca?origin=patient-dashboard&embed=true`
      ...
      <div className="h-full flex flex-col p-4 items-center" style={{ paddingTop: '2%', paddingBottom: '12%' }}>
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden w-full" style={{ height: '80%', maxHeight: '80%' }}>
          <iframe
            src={chatUrl}
            className="w-full h-full border-0"
            title="Chat com Nôa - Avaliação Inicial"
            style={{ backgroundColor: 'transparent' }}
          />
        </div>
      </div>
  ...
  const renderMeusAgendamentos = () => {
    const appointmentsUrl = `/app/clinica/paciente/agendamentos?embed=true`
  ...
  const renderReportProblem = () => {
    return (
      <div className="space-y-6">
        <div className="bg-gradient-to-r from-red-600 to-rose-600 rounded-xl p-6 text-white">
          <div className="flex items-center gap-3 mb-2">
            <AlertCircle className="w-8 h-8" />
            <h2 className="text-2xl font-bold">Reportar Problema</h2>
          </div>
          <p className="text-white/90">Envie uma mensagem curta para a equipe de suporte e administradores</p>
        </div>
  ...
  const renderPerfil = () => {
    ...
    // Calcular dias desde o primeiro acesso (assumindo que o primeiro relatório ou consulta indica o início)
    const primeiroAcesso = reports.length > 0 
      ? new Date(reports.sort((a, b) => new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime())[0].created_at || Date.now())
              ...
              <div className="mt-4 pt-4 border-t border-slate-700/50 space-y-3">
                ...
                <div className="bg-slate-800/50 rounded-lg p-3">
                  {(() => {
                    ...
                    return (
                      <>
                        <div className="text-xs text-slate-300 font-semibold mb-2 text-center">
                            ...
                            return (
                              <div
                                ...
                                className={`aspect-square rounded text-xs flex items-center justify-center ${
                                  isToday
                                    ? 'bg-primary-500/30 text-primary-200 font-bold'
                                    : hasAppointment
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-primary-500/20 to-emerald-500/20 border border-primary-500/40 text-primary-200 hover:from-primary-500/30 hover:to-emerald-500/30 transition-all text-sm font-semibold"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-gradient-to-r from-emerald-500/20 to-primary-500/20 border border-emerald-500/40 text-emerald-200 hover:from-emerald-500/30 hover:to-primary-500/30 transition-all text-sm font-semibold"
                  >
  ...
  const renderDashboard = () => {
    return (
      <div className="space-y-6">
      {/* Cards de Ações Rápidas - REMOVIDOS (agora estão no sidebar) */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 hidden">
        {/* Agendar Consulta */}
        <button
          onClick={handleScheduleAppointment}
  ...
  const renderConteudoEducacional = () => (
    <div className="space-y-7">
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-7 space-y-5">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-5">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-primary-300 mb-3">Biblioteca MedCannLab</p>
          ...
          <div className="flex flex-col items-end gap-3 text-sm uppercase tracking-[0.32em] text-primary-200">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-primary-500/10 border border-primary-500/20 text-primary-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-slate-300">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-xs uppercase tracking-[0.28em] text-primary-300 mb-2">Trilhas guiadas</p>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-xs uppercase tracking-[0.28em] text-primary-300 mb-2">Protocolos clínicos</p>
            <p className="leading-relaxed">
              Sínteses rápidas para consulta durante o acompanhamento: titulação, monitoramento e ajustes personalizados.
            </p>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-xs uppercase tracking-[0.28em] text-primary-300 mb-2">Comunicação terapêutica</p>
            <p className="leading-relaxed">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-7 space-y-5">
        {educationalError && (
          <div className="rounded-xl border border-rose-500/40 bg-rose-950/40 text-rose-200 text-base px-5 py-4">
            {educationalError}
          </div>
        )}
        ...
        {educationalLoading ? (
          ...
          <div className="space-y-5">
            <div className="rounded-xl border border-primary-500/20 bg-primary-500/5 p-6 text-base text-primary-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {BASIC_MODULE_SNIPPETS.map(snippet => (
                <article
                  key={snippet.id}
                  className="rounded-2xl border border-slate-800 bg-slate-950/60 p-6 flex flex-col gap-4 hover:border-primary-500/40 transition-colors"
                >
                  <span className="text-xs uppercase tracking-[0.28em] text-primary-300">{snippet.tag}</span>
                  <h3 className="text-xl font-semibold text-white">{snippet.title}</h3>
                  <p className="text-base text-slate-300 leading-relaxed">{snippet.summary}</p>
                  {snippet.quiz && (
                    <div className="mt-3 space-y-4">
                      <p className="text-sm text-slate-400 uppercase tracking-[0.3em]">Quiz interativo</p>
                      <p className="text-base text-slate-200">{snippet.quiz.question}</p>
                        ...
                        {snippet.quiz.options.map(option => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left text-base rounded-lg border px-4 py-2.5 transition-colors ${
                                isCorrectOption
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-lg border border-primary-500/50 bg-primary-500/10 px-4 py-2.5 text-sm font-semibold text-primary-100 hover:bg-primary-500/20 transition-colors"
                      ...
                      {quizResponses[snippet.id]?.status && snippet.quiz.aiFeedback && (
                        <div
                          className={`flex items-start gap-3 rounded-lg border px-4 py-4 text-base ${
                        ...
                        >
                          ...
                          <div>
                            <p className="font-semibold text-sm uppercase tracking-[0.3em] mb-1.5">IA residente</p>
                            <p>
                              {quizResponses[snippet.id]?.status === 'correct'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {educationalResources.map(resource => {
              ...
              return (
                <article
                  key={resource.id}
                  className="rounded-2xl border border-slate-800 bg-slate-950/60 p-6 flex flex-col gap-5 transition-colors hover:border-primary-500/40"
                >
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center">
                      <Icon className={`w-6 h-6 ${visuals.accent}`} />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {resource.category && (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full border border-slate-700 bg-slate-900/70 text-xs text-slate-300">
                            {resource.category}
                          </span>
                        )}
                      </div>
                      <h3 className="text-xl font-semibold text-white mt-2">{resource.title}</h3>
                      {resource.description && (
                        <p className="text-base text-slate-300 mt-3 line-clamp-3">{resource.description}</p>
                      )}
                    </div>
                  </div>
  ...
  return (
    <div className="flex h-full w-full text-white">
      ...
      {isViewingAsPatient && (
        <div className="fixed top-16 left-0 right-0 bg-yellow-600/20 border-b border-yellow-500/50 p-3 z-50">
      ...
      {/* PatientSidebar */}
      <div className="w-80 bg-slate-900/95 border-r border-slate-800 flex-shrink-0 overflow-y-auto h-full">
        <PatientSidebar
          user={user}
      ...
      {/* Main Content Area */}
      <div 
        className="flex-1 transition-all duration-300 h-full flex flex-col"
        style={{ 
          marginTop: isViewingAsPatient ? '60px' : '0',
          overflow: activeTab === 'chat' || activeTab === 'chat-noa' || activeTab === 'meus-agendamentos' ? 'hidden' : 'auto'
        }}
      >
        {activeTab !== 'dashboard' && (
          <div className="flex-shrink-0" style={{ background: 'rgba(15, 36, 60, 0.75)', borderBottom: '1px solid rgba(28,64,94,0.6)' }}>
              ...
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.35em] text-[#8FA7BF]">
                ...
                <span className="text-[#FFD33D]">
                  {activeTab === 'agendamento'
                    ...
                    ? 'Chat com Médico'
                    : activeTab === 'chat-noa'
                    ? 'Chat com Nôa - Avaliação Inicial'
                    : activeTab === 'perfil'
                    ? 'Meu Perfil'
        ...
        {/* Content */}
        <div className={`${activeTab === 'chat' || activeTab === 'chat-noa' || activeTab === 'meus-agendamentos' ? 'flex-1 flex flex-col min-h-0 overflow-hidden' : 'flex-1 overflow-y-auto p-4 md:p-6'}`}>
          <div className={`${activeTab === 'chat' || activeTab === 'chat-noa' || activeTab === 'meus-agendamentos' ? 'h-full flex flex-col min-h-0 w-full' : 'max-w-7xl mx-auto'}`}>
