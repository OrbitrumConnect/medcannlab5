      ...
      <div className={embedded ? 'max-w-[1800px]' : 'max-w-[1800px] mx-auto'}>
        ...
        {!embedded && (
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-xl font-bold text-brand-text">Literatura</h1>
              <p className="text-xs text-brand-text-muted">PubMed · busca em literatura científica externa</p>
            </div>
          </div>
        )}
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs transition-colors whitespace-nowrap ${
                    isActive
                      ...
                      : 'text-brand-text-muted hover:text-slate-200 hover:bg-slate-800/60 border border-transparent'
                  }`}
                  title={tab.hint}
                >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-10 py-2.5 bg-brand-bg border border-brand-border rounded-lg text-brand-text placeholder-slate-500 focus:outline-none focus:border-indigo-500/50"
                />
                {term && (
                  <button
                    onClick={clearSearch}
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-brand-surface rounded transition-colors"
                    title="Limpar"
                  >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-brand-bg border border-brand-border rounded-lg text-brand-text text-sm focus:outline-none focus:border-indigo-500/50"
                >
        ...
        {loading && (
          <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-8 mb-6 text-center">
            ...
            <p className="text-sm text-brand-text-muted">Buscando no PubMed...</p>
          </div>
        )}
            ...
            <div className="flex items-center justify-center gap-2 flex-wrap max-w-2xl mx-auto">
              {SUGGESTIONS.map(s => (
                <button
                  key={s}
                  onClick={() => setTerm(s)}
                  className="px-3 py-1.5 text-xs bg-slate-800/60 hover:bg-indigo-500/15 border border-brand-border hover:border-indigo-500/40 text-brand-text-secondary hover:text-indigo-200 rounded-full transition-colors"
                >
                  {s}
                </button>
              ))}
        ...
        {!loading && !error && articles.length === 0 && term.trim().length >= 3 && lastSearchedTerm && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-brand-text-muted max-w-md mx-auto leading-relaxed mb-1">
    ...
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4 hover:border-indigo-500/40 hover:bg-slate-800/60 transition-colors">
      <div className="flex items-start gap-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-brand-text leading-snug mb-1">
            {article.title}
          </h3>
      ...
      <div className="ml-7 mt-2 flex items-center gap-2 flex-wrap">
        ...
        {article.journal && (
          <span className="text-[10px] text-brand-text-muted italic">{article.journal}</span>
        )}
        {article.pubdate && (
          <>
            <span className="text-[10px] text-slate-600">·</span>
            <span className="text-[10px] text-brand-text-muted">{article.pubdate}</span>
          </>
        )}
        <span className="text-[10px] text-slate-600">·</span>
