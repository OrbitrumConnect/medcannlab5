    ...
    <div className="space-y-6 p-4 md:p-8 mt-2 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Completude Média', value: `${analytics.avgClinicalScore}%`, icon: TrendingUp, color: 'from-amber-500/20 to-amber-600/10', iconColor: 'text-amber-400' }
                  ...
                  >
                    ...
                    {selectedReport?.id === report.id && (
                      <div className="mt-4 pt-4 border-t border-slate-700/30 space-y-4 animate-in fade-in duration-200">
                        ...
                        {report.content?.scores && (
                          <div className="bg-slate-900/50 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              <ScoreBar label="Equilíbrio dos Dados" value={report.content.scores.symptom_improvement || 0} color="bg-purple-500" />
                              <ScoreBar label="Cobertura de Histórico" value={report.content.scores.quality_of_life || 0} color="bg-amber-500" />
                            </div>
                          </div>
                        )}
                        ...
                        {/* Ações e Selo LGPD */}
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-4 border-t border-slate-700/30 mt-4">
                          <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col md:items-end text-[10px] text-slate-500 space-y-1">
                            <div className="flex items-center gap-2">
                              <span className="flex items-center gap-1 text-emerald-500/80 font-medium">
                                <AlertCircle className="w-3 h-3" /> LGPD Compliant
                              </span>
                              <span className="hidden md:inline text-slate-600">•</span>
                              <span className="flex items-center gap-1 text-blue-400/80 font-medium">
                                <CheckCircle className="w-3 h-3" /> Secure by Design
                              </span>
                            </div>
                            <p>Consentimento validado em: {formatDate(report.generated_at)}</p>
                          </div>
                        </div>
                      </div>
                    )}
