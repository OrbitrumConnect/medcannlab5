...
/**
 * Espelho de `location.state.targetProfessional` — o state do Router some em refresh.
 * Não substitui persistência de verdade: URL direta / outro aparelho → usar BD (`aec_assessment_state` / preferência).
 */
const AEC_SESSION_MAX_AGE_MS = 1000 * 60 * 60 * 8 // 8h — evita profissional “preso” após sessões antigas
...
function readAecTargetFromSession(userId: string | undefined): AecTargetProfessional | null {
  ...
  try {
    const raw = sessionStorage.getItem(aecChatContextKey(userId))
    if (!raw) return null
    const o = JSON.parse(raw) as {
      targetProfessional?: AecTargetProfessional
      updatedAt?: number
    }
    if (typeof o.updatedAt === 'number' && Date.now() - o.updatedAt > AEC_SESSION_MAX_AGE_MS) {
      sessionStorage.removeItem(aecChatContextKey(userId))
      return null
    }
...
function writeAecTargetToSession(userId: string, professional: AecTargetProfessional) {
  if (typeof sessionStorage === 'undefined') return
  try {
    sessionStorage.setItem(
      aecChatContextKey(userId),
      JSON.stringify({ targetProfessional: professional, updatedAt: Date.now() })
    )
  } catch {
    /* quota / private mode */
  }
...
const PatientNOAChat: React.FC = () => {
  ...
  const fromRouterState = React.useMemo((): AecTargetProfessional | null => {
  ...
  const [aecTargetProfessional, setAecTargetProfessional] = React.useState<AecTargetProfessional | null>(null)

  React.useEffect(() => {
    if (!user?.id) {
      setAecTargetProfessional(null)
      return
    }
    if (fromRouterState) {
      setAecTargetProfessional(fromRouterState)
      writeAecTargetToSession(user.id, fromRouterState)
      return
    }
    setAecTargetProfessional(readAecTargetFromSession(user.id))
  }, [fromRouterState, user?.id])
