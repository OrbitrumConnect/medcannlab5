...
const PatientDoctorChat: React.FC = () => {
  ...
  return (
    <div className={`${isEmbedMode ? 'h-full' : 'min-h-[calc(100vh-6rem)]'} bg-slate-950 text-slate-100`}>
      <div className={`${isEmbedMode ? 'h-full px-2 py-2 w-full' : 'max-w-6xl mx-auto px-4 py-6'}`}>
        <div className={`flex flex-col ${isEmbedMode ? 'h-full gap-2' : 'gap-6'}`}>
          {/* Botão Voltar e Header ocultados em modo embed */}
          {!isEmbedMode && (
            <>
              <button
                onClick={() => navigate(-1)}
          ...
          <div className={`grid grid-cols-1 lg:grid-cols-[280px_1fr] ${isEmbedMode ? 'gap-2 h-full min-h-0' : 'gap-6'}`}>
            <aside className={`bg-slate-900/60 border border-slate-800 rounded-2xl p-4 flex flex-col ${isEmbedMode ? 'h-full min-h-0' : ''}`}>
              <h2 className={`${isEmbedMode ? 'text-base' : 'text-sm'} font-semibold text-slate-200 mb-4 flex items-center gap-2`}>
                <Users className={`${isEmbedMode ? 'w-5 h-5' : 'w-4 h-4'} text-primary-400`} />
                Equipe clínica
              </h2>

              {inboxLoading ? (
                <div className={`flex-1 flex items-center justify-center text-slate-400 ${isEmbedMode ? 'text-base' : 'text-sm'}`}>
                  <Loader2 className={`${isEmbedMode ? 'w-5 h-5' : 'w-4 h-4'} animate-spin mr-2`} /> Carregando canais...
                </div>
              ) : patientRooms.length === 0 ? (
                <div className={`flex-1 ${isEmbedMode ? 'text-base' : 'text-sm'} text-slate-400`}>
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex-1 min-w-0">
                            <p className={`${isEmbedMode ? 'text-base' : 'text-sm'} font-semibold truncate`}>{room.name || 'Canal Clínico'}</p>
                            <p className={`${isEmbedMode ? 'text-sm' : 'text-xs'} text-slate-400`}>
                              {room.lastMessageAt
                          ...
                          <div className="flex items-center gap-3 ml-2">
                            {unreadBadge && (
                              <span className={`px-2 py-0.5 rounded-full bg-primary-500 text-white ${isEmbedMode ? 'text-xs' : 'text-[10px]'} font-semibold`}>
                                {room.unreadCount}
                              </span>
                            )}
                            <ChevronRight className={`${isEmbedMode ? 'w-5 h-5' : 'w-4 h-4'} text-slate-500`} />
                          </div>
                        </div>
                      </button>
                    )
            ...
            <section className={`bg-slate-900/60 border border-slate-800 rounded-2xl flex flex-col ${isEmbedMode ? 'h-full' : 'min-h-[480px]'}`}>
              {/* Cabeçalho do canal ocultado em modo embed */}
              {!isEmbedMode && (
              ...
              <div className={`flex-1 overflow-y-auto ${isEmbedMode ? 'p-5' : 'p-4'} space-y-3 custom-scrollbar`}>
                {activeRoomId ? (
                  messagesLoading ? (
                    <div className={`flex items-center justify-center ${isEmbedMode ? 'text-base' : 'text-sm'} text-slate-400 h-full`}>
                      <Loader2 className={`${isEmbedMode ? 'w-5 h-5' : 'w-4 h-4'} animate-spin mr-2`} /> Carregando mensagens...
                    </div>
                  ) : messages.length === 0 ? (
                    <div className={`${isEmbedMode ? 'text-base' : 'text-sm'} text-slate-400 text-center mt-16`}>
                    ...
                    messages.map(msg => {
                      ...
                      return (
                        <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                          <div
                            className={`${isEmbedMode ? 'max-w-2xl' : 'max-w-lg'} rounded-2xl ${isEmbedMode ? 'px-5 py-4' : 'px-4 py-3'} shadow transition-colors ${
                              isOwn
                          ...
                          >
                            <div className="flex items-center justify-between gap-3 mb-1.5">
                              <span className={`${isEmbedMode ? 'text-sm' : 'text-xs'} font-semibold`}>
                                {isOwn ? 'Você' : msg.senderName || 'Profissional'}
                              </span>
                              <span className={`${isEmbedMode ? 'text-xs' : 'text-[10px]'} opacity-70`}>
                            ...
                            <p className={`${isEmbedMode ? 'text-base' : 'text-sm'} whitespace-pre-wrap leading-relaxed`}>{msg.message}</p>
                          </div>
                        </div>
                      )
                    })
                  ...
                  <div className={`flex items-center justify-center h-full ${isEmbedMode ? 'text-base' : 'text-sm'} text-slate-400`}>
                    Selecione um canal de atendimento para visualizar as mensagens.
                  </div>
                )}
              </div>

              <form onSubmit={handleSubmitMessage} className={`border-t border-slate-800 ${isEmbedMode ? 'px-5 py-4' : 'px-4 py-3'}`}>
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className={`flex-1 bg-slate-950/70 border border-slate-800 rounded-xl ${isEmbedMode ? 'px-5 py-4' : 'px-4 py-3'} ${isEmbedMode ? 'text-base' : 'text-sm'} text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500`}
                  />
                  <button
                    type="submit"
                    disabled={!activeRoomId || !messageInput.trim()}
                    className={`inline-flex items-center justify-center ${isEmbedMode ? 'w-12 h-12' : 'w-11 h-11'} rounded-xl bg-primary-600 hover:bg-primary-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    <Send className={`${isEmbedMode ? 'w-5 h-5' : 'w-4 h-4'} text-white`} />
                  </button>
                </div>
              </form>
            </section>
