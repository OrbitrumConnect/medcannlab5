...
const AdminChat: React.FC = () => {
  ...
  if (!user) return <div className="min-h-[60vh] flex items-center justify-center bg-brand-bg text-slate-200"><p>Faça login para acessar o chat.</p></div>
  if (isAuthorizedAdmin === null) return <div className="min-h-[60vh] flex items-center justify-center bg-brand-bg text-slate-200"><Loader2 className="w-6 h-6 animate-spin text-brand-text-muted" /></div>
  if (!isAuthorizedAdmin) return (
    <div className="min-h-[60vh] flex items-center justify-center bg-brand-bg text-slate-200">
      <div className="text-center"><p className="text-xl font-semibold mb-2">Acesso Restrito</p><p className="text-brand-text-muted">Você não tem permissão para acessar este chat.</p></div>
    </div>
  )
  ...
  return (
    <div className="min-h-[calc(100vh-6rem)] bg-slate-950 text-slate-100 flex">
      ...
      {/* Sidebar */}
      <div className={`fixed md:relative inset-y-0 left-0 z-40 w-[280px] max-w-[85vw] md:w-80 md:max-w-none flex flex-col bg-brand-bg border-r border-brand-border-subtle transition-transform duration-200 ease-out ${activeRoomId && !mobileSidebarOpen ? '-translate-x-full md:translate-x-0' : 'translate-x-0'}`}>
        <div className="p-3 md:p-4 border-b border-brand-border-subtle flex items-center justify-between gap-2">
          <h2 className="text-base md:text-lg font-semibold text-brand-text flex items-center gap-2 truncate">
            <Users className="w-5 h-5 text-primary-400 shrink-0" /> Equipe Admin
          </h2>
          <div className="flex items-center gap-1">
            <button type="button" onClick={() => setShowCreateGroup(true)}
              className="p-1.5 rounded-lg text-brand-text-muted hover:text-brand-text hover:bg-brand-surface" title="Criar grupo">
              <Plus className="w-4 h-4" />
            </button>
            {activeRoomId && (
              <button type="button" className="md:hidden p-2 rounded-lg text-brand-text-muted hover:text-brand-text hover:bg-brand-surface"
        ...
        <div className="p-2 md:p-3 border-b border-brand-border-subtle">
          <input type="text" placeholder="Buscar admin..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full px-3 py-2 bg-brand-surface border border-brand-border rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          {adminRooms.filter(r => r.name?.startsWith('👥')).map(room => (
            <button key={room.id} onClick={() => { setActiveRoomId(room.id); navigate(`/app/admin-chat?roomId=${room.id}`, { replace: true }); setMobileSidebarOpen(false) }}
              className={`w-full p-3 rounded-lg text-left transition-colors ${activeRoomId === room.id ? 'bg-primary-600 text-brand-text' : 'bg-slate-800/50 hover:bg-brand-surface text-slate-200'}`}>
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="font-medium truncate">{room.name}</p>
                  <p className="text-xs opacity-75">Grupo</p>
                </div>
                {room.unreadCount > 0 && <span className="bg-primary-500 text-brand-text text-xs rounded-full px-2 py-0.5">{room.unreadCount}</span>}
              </div>
            </button>
          ))}

          {adminsLoading ? (
            <div className="flex items-center justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-brand-text-muted" /></div>
          ) : (
            allAdmins
              ...
              .map(admin => {
                ...
                return (
                  <button key={admin.id}
                    ...
                    className={`w-full p-3 rounded-lg text-left transition-colors ${isActive ? 'bg-primary-600 text-brand-text' : 'bg-slate-800/50 hover:bg-brand-surface text-slate-200'}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
      ...
      {/* Chat Area */}
      <div className="flex-1 flex flex-col bg-brand-bg min-w-0 w-full">
        {!activeRoomId ? (
          <div className="flex-1 flex items-center justify-center text-brand-text-muted">
            <div className="text-center">
              ...
              <button onClick={() => setShowCreateGroup(true)} className="mt-4 px-4 py-2 bg-primary-600 text-brand-text rounded-lg hover:bg-primary-700 transition-colors flex items-center gap-2 mx-auto">
          ...
          <>
            {/* Chat Header */}
            <div className="p-3 md:p-4 border-b border-brand-border-subtle bg-slate-900/50 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-2">
                <button type="button" className="md:hidden p-2 -ml-2 rounded-lg text-brand-text-muted hover:text-brand-text hover:bg-brand-surface shrink-0"
                  onClick={() => setMobileSidebarOpen(true)} aria-label="Abrir lista">
                  <Menu className="w-5 h-5" />
                </button>
                <div className="flex-1 min-w-0">
                  <h3 className="text-base md:text-lg font-semibold text-brand-text truncate">{chatHeader}</h3>
                  <p className="text-sm text-brand-text-muted truncate">{chatSubtitle}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => setShowAddMember(true)} className="p-2 bg-slate-700/50 text-brand-text-secondary rounded-lg hover:bg-brand-surface-subtle transition-colors" title="Adicionar membro">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {messagesLoading ? (
                ...
                <div className="text-sm text-brand-text-muted text-center mt-16">Nenhuma mensagem ainda. Envie a primeira!</div>
              ) : (
                messages.map(msg => {
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-lg rounded-2xl px-4 py-3 shadow transition-colors ${isOwn ? 'bg-primary-600 text-brand-text' : 'bg-brand-surface text-slate-100'}`}>
                        ...
                        <p className={`text-xs mt-1 ${isOwn ? 'text-primary-200' : 'text-brand-text-muted'}`}>{timeDisplay}</p>
                      </div>
                    </div>
                  )
                })
            ...
            {/* Message Input */}
            <div className="p-4 border-t border-brand-border-subtle bg-slate-900/50 backdrop-blur-sm">
              <div className="flex items-center gap-2">
                <input type="file" ref={fileInputRef} accept="image/*" className="hidden"
                  onChange={(e) => { const f = e.target.files?.[0]; if (f) handleImageUpload(f); e.target.value = '' }} />
                <button onClick={() => fileInputRef.current?.click()} disabled={uploadingImage}
                  className="p-2 text-brand-text-muted hover:text-brand-text hover:bg-brand-surface rounded-lg transition-colors disabled:opacity-50" title="Enviar imagem">
                ...
                <input type="text" value={messageInput} onChange={(e) => setMessageInput(e.target.value)}
                  ...
                  className="flex-1 px-4 py-2 bg-brand-surface border border-brand-border rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                <button onClick={handleSendMessage} disabled={!messageInput.trim()}
                  className="p-2 bg-primary-600 text-brand-text rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {showCreateGroup && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowCreateGroup(false)}>
          <div className="bg-brand-surface rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-brand-text flex items-center gap-2"><Users className="w-5 h-5 text-primary-400" /> Criar Grupo</h3>
              <button onClick={() => setShowCreateGroup(false)} className="text-brand-text-muted hover:text-brand-text"><X className="w-5 h-5" /></button>
            </div>
            <input type="text" placeholder="Nome do grupo..." value={groupName} onChange={e => setGroupName(e.target.value)}
              className="w-full px-3 py-2 mb-4 bg-brand-surface-subtle border border-slate-600 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500" />
            <p className="text-sm text-brand-text-muted mb-2">Selecione os membros:</p>
            <div className="space-y-1 mb-4">
              {allAdmins.filter(a => a.id !== user.id).map(admin => (
                <label key={admin.id} className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${selectedGroupMembers.includes(admin.id) ? 'bg-primary-600/20 border border-primary-500/30' : 'hover:bg-brand-surface-subtle'}`}>
                  ...
                  <div><p className="text-sm font-medium text-slate-200">{admin.name || admin.email?.split('@')[0]}</p><p className="text-xs text-brand-text-muted">{admin.email}</p></div>
                </label>
              ))}
            </div>
            <button onClick={handleCreateGroup} disabled={!groupName.trim() || selectedGroupMembers.length === 0}
              className="w-full py-2 bg-primary-600 text-brand-text rounded-lg hover:bg-primary-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium">
      ...
      {showAddMember && activeRoomId && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowAddMember(false)}>
          <div className="bg-brand-surface rounded-2xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-brand-text flex items-center gap-2"><UserPlus className="w-5 h-5 text-primary-400" /> Adicionar Membro</h3>
              <button onClick={() => setShowAddMember(false)} className="text-brand-text-muted hover:text-brand-text"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-sm text-brand-text-muted mb-3">Membros atuais: {participants.map(p => p.name || p.email?.split('@')[0]).join(', ')}</p>
            <div className="space-y-1">
              {allAdmins.filter(a => a.id !== user.id && !participants.some(p => p.id === a.id)).map(admin => (
                <button key={admin.id} onClick={() => handleAddMemberToRoom(admin.id)}
                  className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-brand-surface-subtle transition-colors text-left">
      ...
      {pendingCallRequest && (
        <div className="fixed bottom-4 right-4 bg-blue-600/90 backdrop-blur-sm border border-blue-500/50 rounded-xl p-4 shadow-2xl max-w-sm z-50">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center shrink-0"><Loader2 className="w-5 h-5 text-brand-text animate-spin" /></div>
            ...
            <button onClick={() => setConfirmModal({ isOpen: true, title: 'Cancelar', message: 'Cancelar a solicitação?', type: 'warning',
              ...
              className="text-white/60 hover:text-brand-text p-1 hover:bg-white/10 rounded"><X className="w-4 h-4" /></button>
          </div>
        </div>
      )}
