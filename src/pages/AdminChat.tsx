...
// Admin access now governed by server-side has_role() via user_roles table
...
const AdminChat: React.FC = () => {
  ...
  // Admin auth via server-side RPC (replaces hardcoded ADMIN_EMAILS)
  ...
  // Carregar lista de admins autorizados (via user_roles)
  useEffect(() => {
    const loadAdmins = async () => {
      ...
      try {
        // Buscar apenas usuários com role 'admin' na tabela user_roles
        ...
        if (rolesError || !adminRoles?.length) {
          console.error('Erro ao carregar roles de admin:', rolesError)
          setAllAdmins([])
      ...
      try {
        ...
        if (error) {
          ...
          if (directData && directData.length > 0) {
            ...
            // Map all participants (no hardcoded email filter)
