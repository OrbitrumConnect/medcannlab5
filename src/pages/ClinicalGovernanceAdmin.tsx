...
// --- Space Components ---
const SpaceCard = forwardRef<HTMLDivElement, { children: React.ReactNode, className?: string }>(({ children, className = '' }, ref) => (
    <div ref={ref} className={`bg-[#0B1120]/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl shadow-xl shadow-black/40 ${className}`}>
        {children}
    </div>
))
...
export default function ClinicalGovernanceAdmin() {
    ...
    return (
        <div className="min-h-screen app-bg-gradient p-6 lg:p-10 font-sans text-slate-300">
            ...
            <SpaceCard className="min-h-[600px] flex flex-col">
                ...
                <div className="md:hidden divide-y divide-slate-800/50">
                    {paginatedUsers.map((user) => (
                        <div key={user.user_id} className={`p-4 space-y-3 ${user.status === 'banned' ? 'opacity-50' : ''}`}>
                            <div className="flex items-start gap-3">
                                <div className="flex flex-col items-center gap-1 pt-0.5">
                                    ...
                                    <span className={`text-[9px] font-bold ${user.is_online ? 'text-emerald-400' : 'text-slate-600'}`}>
                                        {user.status === 'banned' ? 'BAN' : user.is_online ? 'ON' : 'OFF'}
                                    </span>
                                </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold ${user.payment_status === 'paid'
                                        ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                                        : 'bg-amber-500/10 border-amber-500/30 text-amber-400'
                                        }`}
                                >
                                    {user.payment_status === 'paid' ? <CheckCircle className="w-3 h-3" /> : <CreditCard className="w-3 h-3" />}
                                    {user.payment_status.toUpperCase()}
                                </button>
                                {user.owner_id && (
                                    <span className="text-[10px] text-slate-500 flex items-center gap-1">
                                        <User className="w-3 h-3" />
                                        {users.find(u => u.user_id === user.owner_id)?.name || '?'}
                                    </span>
                                )}
                                ...
                                <button
                                    ...
                                    className={`p-1 rounded transition-all active:scale-95 ml-1 ${user.status === 'banned'
                                        ? 'text-emerald-400 hover:bg-emerald-500/10'
                                        : 'text-slate-600 hover:text-red-400 hover:bg-red-500/10'
                                        }`}
                                    title={user.status === 'banned' ? 'Desbanir' : 'Banir'}
                                >
                                    {user.status === 'banned' ? <Unlock className="w-3 h-3" /> : <Ban className="w-3 h-3" />}
                                </button>
                            </div>
                        </div>
                    ))}
                ...
                {totalPages > 1 && (
                    <div className="p-4 border-t border-slate-700/50 flex items-center justify-between">
                        <span className="text-xs text-slate-500">
                            {filteredUsers.length} usuários • Página {currentPage}/{totalPages}
                        </span>
