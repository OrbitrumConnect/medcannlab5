...
export class NoaAssistantIntegration {
  ...
  constructor(config: Partial<AssistantConfig>) {
    // SECURITY: OpenAI API key NEVER in frontend. All AI calls go through 
    // tradevision-core Edge Function which holds the key as a Supabase secret.
    this.config = {
      assistantId: config.assistantId || 'asst_CAW142M53uLBLbVzERZMa7HF',
      apiKey: '', // Intentionally empty — forces fallback to local/Edge Function
  ...
  async sendMessage(
    ...
    try {
      ...
      console.info('Todas as chamadas de IA passam pela Edge Function tradevision-core. Fallback local ativado.')
