...
export class PlatformFunctionsModule {
  private static instance: PlatformFunctionsModule
  ...
  detectIntent(
    ...
    // Importante: "modo/modulo paciente" é navegação (UI), não consulta de pacientes.
    // Evita disparar PATIENTS_QUERY quando o usuário só quer trocar de módulo.
    if (
      lowerMessage.includes('modo paciente') ||
      lowerMessage.includes('modulo paciente') ||
      lowerMessage.includes('módulo paciente') ||
      lowerMessage.includes('ver como paciente') ||
      lowerMessage.includes('dashboard paciente')
    ) {
    ...
    try {
      switch (intent.type) {
        ...
        case 'APPOINTMENTS_QUERY':
          return await this.queryAppointments(userId, platformData)
   ...
   */
  /**
   * Iniciar avaliação clínica (Delegado ao AEC Core)
   */
  private async startAssessment(userId: string, platformData?: any): Promise<PlatformActionResult> {
    // Agora o Platform apenas sinaliza o desejo de iniciar. 
    // O ClinicalAssessmentFlow (Core) é a única fonte de verdade para o estado.
    return {
      success: true,
      data: {
      ...
      requiresResponse: false // O Assistant (Nôa) vai responder baseada no estado do Core
    }
  }

  /**
   * Completar avaliação (Processo de finalização delegado)
   */
  private async completeAssessment(
    userId: string,
    assessmentData?: any, // Dados brutos vindos do Core
    platformData?: any
  ): Promise<PlatformActionResult> {
    // A finalização agora é um evento de orquestração.
    // O Core já deve ter processado a lógica clínica.
    ...
    return {
      ...
      requiresResponse: true // Assistant menciona a conclusão
    }
  }

  /**
   * Gerar relatório manualmente (Fallback ou ação direta)
   */
  private async generateReport(userId: string, platformData?: any): Promise<PlatformActionResult> {
    ...
    // Busca o estado real direto do Core antes de gerar
    const assessmentState = clinicalAssessmentFlow.getState(userId)
    
    const report = await clinicalReportService.generateAIReport(
      userId,
      patientName,
      assessmentState?.data || {} // Usa dados REAIS da AEC
    )
  ...
  private async queryPatients(userId: string): Promise<PlatformActionResult> {
    try {
      ...
      if (!assessmentsError && assessments) {
        assessments.forEach(assessment => {
          ...
          if (patientId) {
            ...
            const patientName = assessmentData.name || (assessment as any).patient_name || `Paciente ${patientId.slice(0, 8)}`
      ...
      const { data: users, error: usersError } = await supabase
        .from('users')
        .select('id, name, email, type, cpf, phone')
   ...
   */
  private async queryAppointments(userId: string, platformData?: any): Promise<PlatformActionResult> {
    try {
      // Determinar o papel do usuário para realizar a query correta
      const userType = platformData?.user?.user_type || 'student'
      ...
      if (isPatient) {
        // Se for paciente, busca por patient_id
        appointmentsQuery = appointmentsQuery.eq('patient_id', userId)
      } else {
        // Se for profissional/admin, busca por professional_id (ou doctor_id fallback)
        appointmentsQuery = appointmentsQuery.eq('professional_id', userId)
      }

      appointmentsQuery = appointmentsQuery.order('appointment_date', { ascending: true })

      let { data: appointments, error } = await appointmentsQuery

      if (error && (error.code === '42703' && !isPatient)) {
        // Coluna professional_id inexistente (instância antiga) – tentar doctor_id como fallback APENAS para profissionais
  ...
  /**
   * Atualizar estado da avaliação clínica (Legado)
   */
  async updateAssessmentState(userId: string, state: any): Promise<void> {
    try {
      ...
      if (error) console.warn('⚠️ Erro ao persistir estado legado no banco:', error)
    } catch (e) {
      console.error('Erro ao atualizar estado de avaliação:', e)
    }
  }
}
