...
export class PlatformFunctionsModule {
  ...
  private async queryPatients(userId: string): Promise<PlatformActionResult> {
    try {
      ...
      // PRIMEIRO: Buscar TODOS os pacientes da tabela users (fonte primária de nomes)
      let users: any[] = []
      let usersError: any = null
      
      // Tentar primeiro com 'type' (formato mais comum)
      const { data: usersData, error: usersError1 } = await supabase
        .from('users')
        .select('id, name, email, type, cpf, phone')
        .eq('type', 'patient')
      
      if (!usersError1 && usersData) {
        users = usersData
      } else {
        // Fallback 1: tentar com 'user_type'
        const { data: usersData2, error: usersError2 } = await supabase
          .from('users')
          .select('id, name, email, user_type, cpf, phone')
          .eq('user_type', 'patient')
        
        if (!usersError2 && usersData2) {
          users = usersData2
        } else {
          // Fallback 2: tentar users_compatible
          const { data: usersData3, error: usersError3 } = await supabase
            .from('users_compatible')
            .select('id, name, email, type, cpf, phone')
            .eq('type', 'patient')
          
          if (!usersError3 && usersData3) {
            users = usersData3
          } else {
            usersError = usersError3 || usersError2 || usersError1
            console.warn('⚠️ Erro ao buscar pacientes de users/users_compatible:', usersError)
          }
        }
      }

      // Criar mapa de nomes reais primeiro
      const realNamesMap = new Map<string, { name: string; email?: string; phone?: string; cpf?: string }>()
      if (!usersError && users && users.length > 0) {
        users.forEach(user => {
          if (user.name && user.name.trim() && !user.name.toLowerCase().includes('paciente')) {
            realNamesMap.set(user.id, {
              name: user.name.trim(),
              email: user.email,
              phone: user.phone,
              cpf: user.cpf
      ...
      // 1. Buscar pacientes das avaliações clínicas (usar nomes reais quando disponíveis)
      ...
      if (!assessmentsError && assessments) {
        assessments.forEach(assessment => {
          ...
          if (patientId) {
            ...
            const realPatientInfo = realNamesMap.get(patientId)
            // PRIORIZAR nome real da tabela users
            const patientName = realPatientInfo?.name || 
                               assessmentData.name || 
            ...
            if (!patientsMap.has(patientId)) {
              patientsMap.set(patientId, {
                id: patientId,
                name: patientName,
                cpf: realPatientInfo?.cpf || assessmentData.cpf || '',
                phone: realPatientInfo?.phone || assessmentData.phone || '',
                email: realPatientInfo?.email || assessmentData.email || '',
              ...
              // Atualizar nome apenas se o novo for melhor (nome real)
              if (realPatientInfo?.name && (!patient.name || patient.name.includes('Paciente'))) {
                patient.name = realPatientInfo.name
              }
      ...
      // 2. Adicionar pacientes da tabela users que não estão nas avaliações
      if (!usersError && users && users.length > 0) {
        users.forEach(user => {
          const realPatientInfo = realNamesMap.get(user.id)
          if (!patientsMap.has(user.id)) {
            patientsMap.set(user.id, {
              id: user.id,
              name: realPatientInfo?.name || `Paciente ${user.id.slice(0, 8)}`,
              email: user.email || realPatientInfo?.email,
              cpf: user.cpf || realPatientInfo?.cpf || '',
              phone: user.phone || realPatientInfo?.phone || '',
              status: 'Ativo',
              assessmentCount: 0,
              source: 'users'
            })
          } else {
            // Atualizar dados do usuário se já existe - SEMPRE priorizar nome real
            const patient = patientsMap.get(user.id)
            if (realPatientInfo?.name) {
              patient.name = realPatientInfo.name
            }
            if (user.cpf || realPatientInfo?.cpf) patient.cpf = user.cpf || realPatientInfo?.cpf || patient.cpf
            if (user.phone || realPatientInfo?.phone) patient.phone = user.phone || realPatientInfo?.phone || patient.phone
            if (user.email || realPatientInfo?.email) patient.email = user.email || realPatientInfo?.email || patient.email
            patient.source = 'both'
          }
        })
      }
