...
**Cron `sgq-health-checks-daily` ATIVO 29/05** (V1.9.503): roda diariamente 06h BRT (09h UTC), executa `SELECT public.run_sgq_health_checks()` que checa 5 itens (RLS / PII / AECs órfãs >7d / cron failures 24h / ICP coverage) e grava em `public.system_health_alerts` (fila auditável). Smoke inicial detectou 1 falso positivo PII (regex `[A-Z][a-z]+ [A-Z][a-z]+` pegou "Análise Holística") → dismissed + V1.9.504 refinar regex programado. Total 4 cron jobs agora (era 3). Memória completa: `project_v1_9_503_sgq_health_checks_nivel1_29_05`.
