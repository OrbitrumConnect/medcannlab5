...
- **V1.9.535**: agrupa chat_interaction por dia (Carolina 236 turnos → 1 card resumo + timeline modal expandida)
- **V1.9.536**: tooltip explicado Team Health Score (fórmula 40%+30%+30% visível UI) + bloco alertas inatividade equipe (B1+B7 calibrados de proposta externa 7 blocos, 4 Babylon rejeitados)
- **V1.9.537**: compactar texto educativo TeamManagement (banner amarelo, subtítulo, ReferralsManager) + colapsar Direcionamento vazio (~200px economia)
- **V1.9.538**: background gradient padronizado (.app-bg-gradient utility CSS DRY, 28 substituições em 9 arquivos: IntegratedWorkstation+ResearchWorkstation+4 dashboards) + Agenda KPIs elite (ícones+cores+hover)
- **V1.9.539**: CTAs translucent emerald cristalizado (Library bg local removido + 3 botões PatientsManagement + Anexar arquivo purple→emerald + Prescrição CFM + remove 2 placeholders fake)
- **V1.9.540**: Evolução grupo colapsável + paginação 10/grupo (chat-ia default collapsed, evita Carolina 39 cards empilhados)
- **V1.9.541**: cards Nova Receita paridade visual Workstation (badges coloridos w-10 + tipografia text-sm bold)
- **V1.9.542**: BUG fix useEffect deps (`searchParams` instável react-router → aba Prescrição resetava overview no click — não era cache, era React)
- **V1.9.543** debug wrapper setActiveTab (revertido)
- **V1.9.543-B**: fix loading state Prescrição (4 cards vazios `—` → pill spinner — mesma shape do empty state evita flash)
- **V1.9.544**: padronização `max-w-screen-2xl` (1536px) dashboards principais 4K/QHD (ProfessionalMyDashboard + TeamManagement + PatientDashboard + AlunoDashboard) — Pedro tela 41" estava com 1280px max trava

**Fixes via PAT (sem commit)**:
- **Bug login Flávia tokens NULL** ([feedback_nunca_criar_auth_users_sql_direto_tokens_null_31_05](C:/Users/phpg6/.claude/projects/c--Users-phpg6-Desktop-amigo-connect-hub-main/memory/feedback_nunca_criar_auth_users_sql_direto_tokens_null_31_05.md)): meu workaround V1.9.533 ontem (INSERT SQL direto auth.users via PAT) deixou `confirmation_token/recovery_token/email_change_token_*/phone_change*/email_change/reauthentication_token` com NULL ao invés de `""`. Supabase Auth Service quebrou com `500 unexpected_failure "Database error querying schema"`. Reproduzido empíricamente via API direta. Fix: `UPDATE auth.users SET <tokens> = COALESCE(<tokens>, '')`. Smoke pós-fix: login retornou `access_token` JWT válido. Audit preventivo: 0 outros users afetados. **Princípio meta cristalizado**: NUNCA INSERT SQL direto `auth.users` — SEMPRE Edge `create-patient-auth` V1.9.533 (API `auth.admin.createUser`).

**Recalibrações empíricas honestas (audit Pedro pediu validação profunda)**:
- AECs completas: eu disse "7+" → REAL **4**
- Reports assinados: "102" → REAL **43 total / 25 em 30d**
- cfm_prescriptions: "32" → REAL **52**
- Edges ACTIVE: "14" → REAL **15** (esqueci create-patient-auth V1.9.533)
- Gamification: "desabilitada por flag" → REAL **45 rows ATIVAS** (memória 24/04 stale) — **risco amanhã 01/jun ~3h BRT** cron `monthly-closing-medcannlab` 1ª execução vai processar 45 rows, pode popular `ranking_history` indevidamente
- **Princípio meta cristalizado**: memórias estáticas envelhecem em <30d pré-PMF, validar via PAT antes de afirmar status/configs/toggles

**Padrões visuais oficiais cristalizados** ([reference_padrao_visual_app_translucent_emerald_max_w_screen_2xl_31_05](C:/Users/phpg6/.claude/projects/c--Users-phpg6-Desktop-amigo-connect-hub-main/memory/reference_padrao_visual_app_translucent_emerald_max_w_screen_2xl_31_05.md)): (1) CTA translucent emerald, (2) Container max-w-screen-2xl, (3) Background .app-bg-gradient utility, (4) Loading state shape consistente com empty, (5) Render condicional progressivo.

**Estado financeiro empírico** ([feedback_paymentgate_ausente_stripe_mock_pre_marco_2_31_05](C:/Users/phpg6/.claude/projects/c--Users-phpg6-Desktop-amigo-connect-hub-main/memory/feedback_paymentgate_ausente_stripe_mock_pre_marco_2_31_05.md)): sistema DEMO/MOCK — Stripe NÃO conectado + Gate hard NÃO existe enforced + UI/RPC admin liberação NÃO existe. 51 users com payment_status setado MANUALMENTE via DB (40 paid + 7 exempt + 4 pending), 0 user_subscriptions ativas, 0 patient_referrals reais. Faz sentido pré-PMF (zero pagantes externos + CNPJ João pendente Marco 1).

**HEAD anterior**: `59a1313` → série V1.9.534-544 → atual `cae44e9`. Locks preservados V1.9.95+97+98+99-B+299+388-A.3+452+468-B + sessões 27→29/05 entregaram Sprint D Matrix Camadas 1.X + Sprint E 3 verticais Ensino + V1.9.452 PII sanitize fechado + SGQ ANVISA (20 drafts em `docs/sgq/`). **HEAD atual `07d7520`** (30/05 ~17h, docs trail DIARIO_30 PARTE 2 + acordo v2.1 RASCUNHO + CLAUDE.md atualizado). **Entregas 30/05**: V1.9.504 housekeeping + V1.9.505 PII regex refined + V1.9.506 Sprint A `verify_jwt` restaurado tradevision-core + V1.9.507-514 8 commits UX prontuário/sidebar/aluno + **V1.9.515 fix bug crítico InterruptedAECsCard** (Sprint A V1.9.500 estava em ProfessionalDashboard.tsx LEGACY → movido pra ProfessionalMyDashboard.tsx ATIVO via screenshot empírico Pedro) + **V1.9.516 Check 6 AEC pending followup >5d** (Check 6 em `run_sgq_health_checks()` reuso pattern V1.9.503/505, detectou caso real Illa Proença COMPLAINT_DETAILS 7d) + **V1.9.517+518+519 cleanup 3 Edges órfãs** (get_chat_history + google-auth + sync-gcal flip verify_jwt false→true via Management API + smoke 6/6 PASS, janela observação 48h, decisão hard-delete batch 01/jun segunda ~15h BRT). Tags ativas: `v1.9.452-pii-sanitize-defensivo-final` + `v1.9.468-A-matrix-bula-locks-final` + `v1.9.299-pbad-conforme-locked` (entre 11 total). Pré-PMF declarado empíricamente (0 pacientes externos pagantes; **Eduardo Faveret virou usuário operacional 27/05 laptop**; testes internos + 1 paciente externa real confirmada Ricardo: Maria das Dores Pinto Pitoco com 3 racionalidades). **Sessão societária 30/05**: descoberta `acordo_quotistas_juridico.md` v2.0 existente + redação `acordo_quotistas_juridico_v2_1_RASCUNHO.md` (Minuta pra Revisão Jurídica, 350+ linhas, 13 cláusulas + 5 anexos, 2 rounds auditoria externa Claude2+Ricardo, **NÃO promovido oficial** — aguarda reunião 4 sócios + Paulo contador + advogado societário + advogado saúde digital).

**P0 V1.9.452 LGPD sanitize ✅ RESOLVIDO 29/05** (`8ed1802`) + **V1.9.597 reforço 05/06** (`4982306`): Edge `tradevision-core` v425 (V1.9.566) com `sanitizeRationalityPII` accent-insensitive + `lookupPatientName` patcheando 2 INSERTs + backfill 132 rows via PL/pgSQL. Audit empírico **05/06 via PAT** (heurística TOKENIZAÇÃO de `users.name` no banco vivo — não lista fechada) revelou refinamento: **142 rows total / 121 (85%) com `Paciente #XXXXXX` explícito + 21 com linguagem genérica ("o paciente apresenta") + 2 com nome do meio "Cristina" escapou sanitize por typo "Critstina" no users.name da Flávia**. V1.9.597 corrigiu as 2 da Flávia via REGEXP_REPLACE word-boundary → **0 nomes reais cruzados HOJE**. Marco 2 não fica bloqueado. Detalhes: DIARIO_30 §B.2 + memória `feedback_sanitize_pii_nome_meio_escapa_05_06` (going-forward: Edge v426+ tolerar typo).
...
🟢 CORE / FUNCIONAIS (verify_jwt=true — defesa em camadas restaurada)
  ...
  digital-signature             v68   Assinatura digital ICP-Brasil/CFM 3 levels (SIMULAÇÃO-default; 0 SIM- no banco — PBAD REAL só em sign-pdf-icp Lock V1.9.299) [V1.9.521]
