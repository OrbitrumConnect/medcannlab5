...
DROP POLICY IF EXISTS "Pacientes podem ver seus próprios relatórios" ON clinical_reports;
DROP POLICY IF EXISTS "Profissionais podem ver todos os relatórios" ON clinical_reports;
...
-- CORRIGIDO: Comparação direta de TEXT com UUID convertido para TEXT
...
CREATE POLICY "Profissionais podem ver todos os relatórios" ON clinical_reports
  FOR ALL USING (
    EXISTS (
      ...
      WHERE id = auth.uid()
      AND (
        raw_user_meta_data->>'type' IN ('professional', 'admin', 'profissional')
        OR email = 'rrvalenca@gmail.com'
        OR email = 'eduardoscfaveret@gmail.com'
      )
    )
  );
...
DROP POLICY IF EXISTS "Sistema pode inserir notificações" ON notifications;
